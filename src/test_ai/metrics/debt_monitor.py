...
class SystemAuditor:
    ...
    def __init__(
        ...
        self._check_functions: dict[
            str, Callable[..., Coroutine[Any, Any, AuditResult]]
        ] = {}
    ...
    if config_path.exists():
        for config_file in config_path.glob("*.yaml"):
            ...
            try:
                ...
                config_snapshots[config_file.name] = yaml.safe_load(
...
def load_active_baseline(backend: DatabaseBackend) -> SystemBaseline | None:
    ...
    return SystemBaseline(
        ...
        config_snapshots=json.loads(row["config_snapshots"])
        if row["config_snapshots"]
        else {},
        package_versions=json.loads(row["package_versions"])
        if row["package_versions"]
        else {},
    )
