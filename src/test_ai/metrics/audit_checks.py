...
def _diff_configs(
    ...
    if not isinstance(expected, dict) or not isinstance(actual, dict):
        if expected != actual:
            diffs.append(
                {"path": path or "<root>", "expected": expected, "actual": actual}
            )
        return diffs
...
def register_default_checks(
    ...
    async def _check_task_time(check: AuditCheck) -> AuditResult:
        return await check_task_completion_time(
            check, backend=backend, baseline=baseline
        )
    ...
    async def _check_skills(check: AuditCheck) -> AuditResult:
        return await check_skill_integrity(
            check, baseline=baseline, skills_path=skills_path
        )

    async def _check_config(check: AuditCheck) -> AuditResult:
        return await check_config_drift(
            check, baseline=baseline, config_path=config_path
        )
