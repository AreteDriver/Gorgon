...
def validate_identifier(
    ...
    if len(value) > max_length:
        raise ValidationError(
            f"{name} exceeds maximum length of {max_length}: {len(value)}"
        )
    ...
    if step_type == "shell":
        ...
        if command:
            validate_shell_command(
                command, allow_dangerous=params.get("allow_dangerous", False)
            )
        ...
        if len(prompt) > 100000:  # 100KB limit
            raise ValidationError(
                f"Prompt exceeds maximum length: {len(prompt)} > 100000"
            )
