...
class ParallelExecutionMetrics:
    ...
    def _finalize(self) -> None:
        ...
        self.successful_count = sum(
            1 for b in self.branches.values() if b.status == "success"
        )
        self.failed_count = sum(
            1 for b in self.branches.values() if b.status == "failed"
        )
        self.cancelled_count = sum(
            1 for b in self.branches.values() if b.status == "cancelled"
        )
    ...
    def completion_ratio(self) -> float:
        ...
        if not self.branches:
            return 0.0
        completed = sum(
            1 for b in self.branches.values() if b.status in ("success", "failed")
        )
...
class RateLimitState:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "avg_wait_ms": (
                self.total_wait_ms / self.total_requests
...
class ParallelExecutionTracker:
    ...
    def register_callback(
        self, callback: Callable[[str, ParallelExecutionMetrics], None]
    ) -> None:
    ...
    def get_summary(self) -> dict:
        ...
        with self._lock:
            ...
            total_executions = (
                self._counters["executions_completed"]
                + self._counters["executions_failed"]
            )
            success_rate = (
            ...
            return {
                ...
                "execution_duration": histogram_stats(
                    self._timings["execution_duration_ms"]
                ),
