...
from test_ai.metrics.cost_tracker import CostTracker
...
class ProviderSelection:
    ...
    reason: str
    confidence: float
    fallback: Optional[ProviderSelection] = None
...
class ProviderRouter:
    ...
    def select_provider(
        ...
        """
        active_strategy = (
            RoutingStrategy(strategy) if strategy else self._default_strategy
        )
        logger.info(
        ...
        candidates = self._build_candidate_list(
            agent_role, task_complexity, budget_remaining
        )

        if not candidates:
            logger.warning(
                "No candidates available for role=%s; returning default", agent_role
            )
    ...
    def update_after_execution(
        ...
        if "quality_score" in outcome:
            stats.quality = (
                alpha * outcome["quality_score"] + (1 - alpha) * stats.quality
            )

        if "latency_seconds" in outcome:
            stats.latency = (
                alpha * outcome["latency_seconds"] + (1 - alpha) * stats.latency
            )
    ...
    def _get_stats(self, agent_role: str, provider: str, model: str) -> _EMAStats:
        ...
        try:
            ...
            if provider_stats and provider in provider_stats:
                ...
                return _EMAStats(
                    quality=ps.get(
                        "quality_score", _DEFAULT_QUALITY.get((provider, model), 0.7)
                    ),
                    latency=ps.get(
                        "avg_latency", _DEFAULT_LATENCY.get((provider, model), 1.0)
                    ),
                    cost=ps.get(
                        "avg_cost", _DEFAULT_COST_PER_1K.get((provider, model), 0.005)
                    ),
