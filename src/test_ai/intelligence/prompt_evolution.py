...
class PromptEvolution:
    ...
    def select_variant(
        ...
        with self._lock:
            ...
            if not variants:
                raise KeyError(
                    f"No variants registered for base template '{base_template_id}'"
                )

            # Check if any variant has outcome data
            has_data = any(
                self._stats.get(v.variant_id, _RunningStats()).trials > 0
                for v in variants
            )
    ...
    def get_variant_report(self, base_template_id: str) -> VariantReport:
        ...
        if len(ranked) >= 2:
            ...
            second = ranked[1]
            both_sufficient = (
                best.trials >= _MIN_TRIALS and second.trials >= _MIN_TRIALS
            )
    ...
    def promote_winner(self, base_template_id: str) -> str | None:
        ...
        if report.winner is None:
            logger.info(
                "No clear winner for '%s'; skipping promotion", base_template_id
            )
            return None
    ...
    def evolve_prompt(
        ...
        if failure_rate < 0.2 and low_quality_rate < 0.2:
            logger.debug(
                "Outcomes for '%s' are acceptable (fail=%.0f%%, low_q=%.0f%%); "
                "no evolution needed",
                base_template_id,
                failure_rate * 100,
