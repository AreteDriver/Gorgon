...
def _evaluate_condition(condition: dict[str, Any], payload: dict) -> bool:
    ...
    if "contains" in condition:
        return (
            condition["contains"] in value if isinstance(value, (str, list)) else False
        )
    if "exists" in condition:
        return (value is not None) == condition["exists"]
    if "in" in condition:
        return (
            value in condition["in"]
            if isinstance(condition["in"], (list, tuple))
            else False
        )
...
class IntegrationGraph:
    ...
    def _dispatch_trigger(
        self, trigger: TriggerRule, payload: dict[str, Any]
    ) -> DispatchResult:
        ...
        if self._dispatch_callback:
            try:
                ...
                logger.error(
                    "Dispatch callback failed for trigger %s: %s", trigger.id, e
                )
    ...
    def register_chain(
        ...
        logger.info(
            "Registered chain %s (%s) with %d steps", chain_id, name, len(rules)
        )
        return chain_id
    ...
    def get_graph(self) -> dict[str, Any]:
        ...
        return {
            ...
            "edges": edges,
            "chains": [
                {"id": c.id, "name": c.name, "steps": len(c.steps)}
