...
class FeedbackEngine:
    ...
    def analyze_agent_trajectory(
        self, agent_role: str, days: int = 30
    ) -> AgentTrajectory:
        ...
        prev_rate = (
            sum(1 for r in earlier if r.success) / len(earlier) if earlier else 0.0
        )
    ...
    def suggest_workflow_improvements(self, workflow_id: str) -> list[Suggestion]:
        ...
        for role, records in step_stats.items():
            ...
            if (
                role_rate == 1.0
                and cost_fraction > _HIGH_COST_FRACTION
                and role_total >= 3
            ):
                suggestions.append(
                    Suggestion(
            ...
            if len(models_used) >= 2:
                ...
                if (
                    cheapest_rate >= expensive_rate - 0.05
                    and cheapest_cost < expensive_cost * 0.7
                ):
                    suggestions.append(
                        Suggestion(
                            step_id=records[-1].step_id,
        ...
        if len(outcomes) >= 2:
            for i in range(len(outcomes) - 1):
                ...
                if (
                    ...
                    a_rate = sum(
                        1 for r in step_stats[a.agent_role] if r.success
                    ) / len(step_stats[a.agent_role])
                    b_rate = sum(
                        1 for r in step_stats[b.agent_role] if r.success
                    ) / len(step_stats[b.agent_role])
        ...
        suggestions.sort(
            key=lambda s: (impact_order.get(s.estimated_impact, 3), -s.confidence)
        )
    ...
    def _evaluate_notability(
        ...
        if (
            stats.avg_cost_usd > 0
            and cost_usd > stats.avg_cost_usd * _COST_MULTIPLIER_THRESHOLD
        ):
            return (
                True,
