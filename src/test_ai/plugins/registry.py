...
class PluginRegistry:
    ...
    def register(self, plugin: Plugin, config: dict | None = None) -> None:
        ...
        with self._lock:
            ...
            self._hooks[PluginHook.WORKFLOW_START].append(
                (plugin.name, plugin.on_workflow_start)
            )
            self._hooks[PluginHook.WORKFLOW_END].append(
                (plugin.name, plugin.on_workflow_end)
            )
            self._hooks[PluginHook.WORKFLOW_ERROR].append(
                (plugin.name, plugin.on_workflow_error)
            )
    ...
    def unregister(self, plugin_name: str) -> bool:
        ...
        with self._lock:
            ...
            to_remove = [
                step_type
                for step_type, (name, _) in self._handlers.items()
                if name == plugin_name
            ]
    ...
    def list_handlers(self) -> dict[str, str]:
        ...
        with self._lock:
            return {
                step_type: plugin_name
                for step_type, (plugin_name, _) in self._handlers.items()
            }
