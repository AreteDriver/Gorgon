...
try:
    from test_ai.mcp.manager import MCPConnectorManager
...
def _render_tool_discovery(manager: MCPConnectorManager) -> None:
    ...
    if st.button("Discover Tools"):
        ...
        if result.success:
            st.success(
                f"Discovered {len(result.tools)} tool(s) and "
                f"{len(result.resources)} resource(s)"
            )
        else:
...
def _render_tool_execution(manager: MCPConnectorManager) -> None:
    ...
    selected_tool = next(
        (t for t in server.tools if t.name == selected_tool_name), None
    )
    ...
    if st.button("Execute Tool", type="primary"):
        with st.spinner(f"Executing {selected_tool_name}..."):
            try:
                from test_ai.mcp.client import HAS_MCP_SDK, call_mcp_tool

                if not HAS_MCP_SDK:
                    st.warning(
                        "MCP SDK not installed. Tool execution requires "
