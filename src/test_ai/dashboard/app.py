...
def render_workflows_page():
    ...
    with tab2:
        ...
        wf_description = st.text_area(
            "Description", placeholder="Describe what this workflow does"
        )

        st.subheader("Steps")

        num_steps = st.number_input(
            "Number of Steps", min_value=1, max_value=10, value=1
        )
        ...
        for i in range(num_steps):
            ...
            with col1:
                step_id = st.text_input(
                    "Step ID", key=f"step_id_{i}", value=f"step_{i + 1}"
                )
            ...
            with col2:
                ...
                next_step = st.text_input(
                    "Next Step ID (optional)", key=f"next_step_{i}"
                )
        ...
        if st.button("💾 Save Workflow", type="primary"):
            if wf_id and wf_name:
                workflow = Workflow(
                    id=wf_id, name=wf_name, description=wf_description, steps=steps
                )
