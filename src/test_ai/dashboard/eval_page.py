...
def render_eval_page() -> None:
    ...
    try:
        from test_ai.evaluation.store import EvalStore
        from test_ai.state.database import get_database
    ...
    with col_filter1:
        agent_role = st.text_input(
            "Agent Role Filter", value="", placeholder="e.g. planner"
        )
    with col_filter2:
    ...
    if not runs:
        st.info(
            "No evaluation runs found. Run `gorgon eval run <suite> --mock` to get started."
        )
        return

    total_runs = len(runs)
    avg_score = (
        sum(r.get("avg_score", 0) for r in runs) / total_runs if total_runs else 0
    )
    avg_pass_rate = (
        sum(r.get("pass_rate", 0) for r in runs) / total_runs if total_runs else 0
    )
