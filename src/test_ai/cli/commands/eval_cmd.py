...
def eval_run(
    ...
    parallel: bool = typer.Option(
        False, "--parallel", "-p", help="Run cases in parallel"
    ),
    output: Optional[str] = typer.Option(
        None, "--output", "-o", help="Save JSON report to file"
    ),
    mock: bool = typer.Option(
        False, "--mock", help="Use mock evaluator (no API calls)"
    ),
    suites_dir: Optional[str] = typer.Option(
        None, "--suites-dir", help="Custom suites directory"
    ),
) -> None:
    """Run an evaluation suite against an agent."""
    from test_ai.evaluation.loader import SuiteLoader
    ...
    # Build evaluator
    if mock:
        from test_ai.evaluation.base import AgentEvaluator
        ...
        evaluator = AgentEvaluator(
            agent_fn=lambda prompt: (
                provider.complete(
                    __import__(
                        "test_ai.providers.base", fromlist=["CompletionRequest"]
        ...
        from test_ai.evaluation.base import ProviderEvaluator

        try:
            from test_ai.providers import get_provider
    ...
    # Run
    from test_ai.evaluation.runner import EvalRunner
    ...
    def progress_callback(current: int, total_count: int, result: object) -> None:
        ...
        console.print(
            f"  [{current}/{total_count}] {status_char}", end="\r", highlight=False
        )
    ...
    # Report
    from test_ai.evaluation.reporters import ConsoleReporter
    ...
    if output:
        from test_ai.evaluation.reporters import JSONReporter
    ...
    try:
        from test_ai.evaluation.store import get_eval_store
...
def eval_results(
    ...
    agent: Optional[str] = typer.Option(
        None, "--agent", "-a", help="Filter by agent role"
    ),
