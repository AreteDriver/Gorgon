...
def health(
    ...
    try:
        from test_ai.agents.convergence import HAS_CONVERGENT

        if not HAS_CONVERGENT:
            console.print("[yellow]Convergent not installed[/yellow]")
            raise typer.Exit(1)

        from test_ai.agents.convergence import create_bridge, get_coordination_health
...
def cycles(
    ...
    try:
        ...
        from test_ai.agents.convergence import check_dependency_cycles
    ...
    try:
        ...
        if event_type is not None:
            try:
                ...
                console.print(
                    f"[red]Unknown event type: {event_type}[/red]\nValid types: {valid}"
                )
                raise typer.Exit(1)
