...
from .graph_models import WorkflowGraph, GraphEdge
...
class GraphWalker:
    ...
    def evaluate_branch(
        ...
        logger.debug(
            f"Branch {node_id}: {field} {operator} {value} = {result} "
            f"(actual: {actual})"
        )
    ...
    def should_continue_loop(
        ...
        if iteration >= max_iterations:
            logger.debug(
                f"Loop {node_id}: stopping at max iterations ({max_iterations})"
            )
            return False
        ...
        if loop_type == "count":
            ...
            logger.debug(
                f"Loop {node_id} iteration {iteration}: "
                f"{field} {operator} {value} = {result}"
            )
            return result
