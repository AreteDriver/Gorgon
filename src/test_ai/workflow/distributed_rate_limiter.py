...
class RateLimitResult:
    ...
    retry_after: Optional[float] = None  # Seconds to wait if not allowed
...
class SQLiteRateLimiter(DistributedRateLimiter):
    ...
    async def acquire(
        ...
        return await loop.run_in_executor(
            None, self._acquire_sync, key, limit, window_seconds
        )
    ...
    async def get_current(self, key: str, window_seconds: int = 60) -> int:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._get_current_sync, key, window_seconds
        )
    ...
    async def cleanup_expired(self, older_than_seconds: int = 3600) -> int:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._cleanup_expired_sync, older_than_seconds
        )
