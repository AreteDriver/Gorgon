...
class BruteForceProtection:
    ...
    def _calculate_block_duration(self, record: AttemptRecord) -> float:
        ...
        duration = self.config.initial_block_seconds * (
            self.config.block_multiplier**block_count
        )
            ...
            if record.attempts >= max_per_hour:
                ...
                if record.failed_attempts >= self.config.max_failed_attempts:
                    record.blocked_until = now + (
                        self.config.failed_attempt_block_hours * 3600
                    )
                    logger.warning(
                        f"Extended block for {identifier}: "
                        f"{record.failed_attempts} failed attempts"
                    )
...
class BruteForceMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        allowed, retry_after = await self.protection.check_allowed_async(
            identifier, is_auth
        )
