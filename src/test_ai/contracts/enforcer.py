...
from test_ai.contracts.base import AgentRole, ContractViolation
from test_ai.contracts.definitions import get_contract
...
class ContractEnforcer:
        ...
        for attempt in range(1, max_retries + 2):  # 1 initial + max_retries
            try:
                ...
                correction_prompt = self.build_correction_prompt(
                    original_prompt, exc, attempt
                )

                try:
                    best_output = await executor_callback(
                        correction_prompt, step_config
                    )
                except Exception:
                    logger.exception(
    ...
    def build_correction_prompt(
        ...
        parts.append(
            "Please try again with a corrected output that satisfies the schema."
        )
    ...
    def get_enforcement_stats(self) -> EnforcementStats:
        ...
        violation_rate = (
            total_violations / total_validations if total_validations > 0 else 0.0
        )
        correction_rate = (
            total_corrections / total_violations if total_violations > 0 else 0.0
        )
