...
class EvalRunner:
    ...
    def run(
        ...
        if filter_tags:
            cases = [
                c
                for c in cases
                if any(tag in c.metadata.get("tags", []) for tag in filter_tags)
            ]
    ...
    def _run_parallel(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [
                executor.submit(evaluate_case, i, case) for i, case in enumerate(cases)
            ]
    ...
    async def _run_parallel_async(
        ...
        async def evaluate_with_index(
            idx: int, case: EvalCase
    ...
    def _aggregate_results(
        self, suite: EvalSuite, results: list[EvalResult]
    ) -> SuiteResult:
