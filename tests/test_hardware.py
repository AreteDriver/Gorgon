...
from test_ai.providers.hardware import (
    HardwareProfile,
    detect_hardware,
    _detect_nvidia_gpu,
...
class TestDetectHardwareNoPsutil:
    def test_returns_degraded_profile(self):
        with patch("test_ai.providers.hardware.HAS_PSUTIL", False):
...
class TestDetectHardwareAppleSilicon:
    ...
    def _detect(self, total_gb: float, avail_gb: float = None):
        ...
        with (
            patch("test_ai.providers.hardware.HAS_PSUTIL", True),
            patch("test_ai.providers.hardware.psutil", mock_ps),
            patch("test_ai.providers.hardware.platform") as mock_platform,
        ):
...
class TestDetectHardwareNvidia:
    ...
    def _detect(self, gpu_name: str, vram_mb: float, sys_total_gb: float = 64.0):
        ...
        with (
            ...
            patch("test_ai.providers.hardware.psutil", mock_ps),
            patch("test_ai.providers.hardware.platform") as mock_platform,
            patch("test_ai.providers.hardware.subprocess") as mock_sub,
        ):
...
class TestDetectNvidiaGpu:
    def test_nvidia_smi_not_found(self):
        with patch("test_ai.providers.hardware.subprocess") as mock_sub:
            mock_sub.run.side_effect = FileNotFoundError
            mock_sub.TimeoutExpired = subprocess.TimeoutExpired
