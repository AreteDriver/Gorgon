...
class TestAgentActivityRoles:
    ...
    def test_more_than_10_agents_shows_last_10(self, mock_streamlit):
        ...
        agent_calls = [
            c
            for c in mock_streamlit.markdown.call_args_list
            if "border-bottom" in str(c)
        ]
...
class TestStatusDicts:
    ...
    def test_all_statuses_have_colors(self, mock_streamlit):
        from test_ai.dashboard.workflow_visualizer import STATUS_COLORS, StepStatus

        for status in StepStatus:
            assert status in STATUS_COLORS

    def test_all_statuses_have_icons(self, mock_streamlit):
        from test_ai.dashboard.workflow_visualizer import STATUS_ICONS, StepStatus

        for status in StepStatus:
            assert status in STATUS_ICONS

    def test_step_type_icons(self, mock_streamlit):
        from test_ai.dashboard.workflow_visualizer import STEP_TYPE_ICONS
