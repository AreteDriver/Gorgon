...
class TestOllamaIsConfigured:
    def test_no_httpx(self):
        ...
        with patch("test_ai.providers.ollama_provider.httpx", None):
            p = OllamaProvider()
    ...
    def test_server_ok(self):
        ...
        with patch("test_ai.providers.ollama_provider.httpx", mock_httpx):
            p = OllamaProvider()
...
class TestOllamaComplete:
    ...
    def test_auto_initialize(self):
        ...
        mock_client.post.return_value = MagicMock(
            json=MagicMock(return_value=resp_json)
        )
    ...
    def test_rate_limit(self):
        ...
        err = real_httpx.HTTPStatusError(
            "rate", request=MagicMock(), response=mock_resp
        )
        p._client.post.return_value = MagicMock(
...
class TestOllamaSelectModelForTier:
    ...
    def test_base_match(self):
        ...
        p = OllamaProvider(tier_models={"fast": ["phi3"]})
        result = p.select_model_for_tier(
            ModelTier.FAST, available_models=["phi3:latest"]
        )
        assert result == "phi3"
    ...
    def test_tagged_candidate_no_base_match(self):
        ...
        result = p.select_model_for_tier(
            ModelTier.FAST, available_models=["qwen2.5:latest"]
        )
        assert result is None
...
class TestBedrockProviderInit:
    def test_default_config(self):
        from test_ai.providers.bedrock_provider import BedrockProvider
...
class TestBedrockIsConfigured:
    def test_no_boto3(self):
        ...
        with patch("test_ai.providers.bedrock_provider.boto3", None):
            p = BedrockProvider()
            assert p.is_configured() is False
    ...
    def test_credentials_found(self):
        ...
        with patch("test_ai.providers.bedrock_provider.boto3", mock_boto):
            p = BedrockProvider()
            assert p.is_configured() is True
...
class TestBedrockModelHelpers:
    def test_resolve_known(self):
        ...
        p = BedrockProvider()
        assert (
            p._resolve_model_id("claude-3-5-sonnet")
...
class TestBedrockParseResponse:
    ...
    def test_amazon(self):
        p = self._provider()
        content, _ = p._parse_response(
            "amazon.titan", {"results": [{"outputText": "t"}]}
        )
        assert content == "t"
...
class TestBedrockComplete:
    ...
    def test_throttling(self):
        ...
        with patch("test_ai.providers.bedrock_provider.ClientError", type(err)):
    ...
    def test_generic_error(self):
        ...
        with patch("test_ai.providers.bedrock_provider.ClientError", _narrow):
...
class TestBedrockStream:
    ...
    def test_stream_anthropic(self):
        ...
        p._runtime_client.invoke_model_with_response_stream.return_value = {
            "body": [event]
        }
    ...
    def test_stream_generic_error(self):
        p = self._make_provider()
        _narrow = type("NarrowClientError", (Exception,), {})
        p._runtime_client.invoke_model_with_response_stream.side_effect = RuntimeError(
            "boom"
        )
        with patch("test_ai.providers.bedrock_provider.ClientError", _narrow):
            with pytest.raises(ProviderError, match="streaming error"):
...
class TestVertexProviderInit:
    def test_default_config(self):
        from test_ai.providers.vertex_provider import VertexProvider
...
class TestVertexIsConfigured:
    def test_no_vertexai(self):
        ...
        with patch("test_ai.providers.vertex_provider.vertexai", None):
            p = VertexProvider()
            assert p.is_configured() is False
    ...
    def test_configured(self):
        ...
        with (
            patch("test_ai.providers.vertex_provider.vertexai", mock_vertexai),
            patch(
                "test_ai.providers.vertex_provider.get_settings",
            ...
            with patch.dict(
                "sys.modules",
                {
                    "test_ai.config.settings": MagicMock(
    ...
    def test_exception_returns_false(self):
        ...
        mock_vertexai = MagicMock()
        with patch("test_ai.providers.vertex_provider.vertexai", mock_vertexai):
            with patch.dict(
                "sys.modules",
                {
                    "test_ai.config.settings": MagicMock(
...
class TestVertexInitialize:
    ...
    def test_success(self):
        ...
        with (
            patch("test_ai.providers.vertex_provider.vertexai", mock_vertexai),
            patch.dict("sys.modules", {"test_ai.config.settings": settings_mod}),
        ):
            p = VertexProvider(project="proj")
            p.initialize()
            assert p._initialized is True
...
class TestVertexComplete:
    ...
    def test_success(self):
        ...
        with (
            patch("test_ai.providers.vertex_provider.GenerativeModel", mock_gen_model),
            patch(
                "test_ai.providers.vertex_provider.GenerationConfig", mock_gen_config
            ),
        ):
            result = p.complete(_basic_request())
    ...
    def test_rate_limit(self):
        ...
        with (
            ...
            patch("test_ai.providers.vertex_provider.ResourceExhausted", mock_re),
        ):
    ...
    def test_google_api_error(self):
        ...
        with (
            ...
            patch("test_ai.providers.vertex_provider.ResourceExhausted", mock_re),
            patch("test_ai.providers.vertex_provider.GoogleAPIError", mock_ge),
        ):
        ...
        with (
            ...
            async def _collect():
                ...
                async for c in p.complete_stream_async(
                    _basic_request(system_prompt="sys")
                ):
                    result.append(c)
                return result
...
class TestVertexGetModel:
    def test_caching(self):
        ...
        mock_gen = MagicMock()
        with patch("test_ai.providers.vertex_provider.GenerativeModel", mock_gen):
            p = VertexProvider()
...
class TestAzureOpenAIProviderInit:
    def test_default_config(self):
        from test_ai.providers.azure_openai_provider import AzureOpenAIProvider
...
class TestAzureIsConfigured:
    def test_no_openai(self):
        ...
        with patch("test_ai.providers.azure_openai_provider.AzureOpenAI", None):
    ...
    def test_configured(self):
        ...
        mock_cls = MagicMock()
        with patch("test_ai.providers.azure_openai_provider.AzureOpenAI", mock_cls):
            p = AzureOpenAIProvider(api_key="k", endpoint="https://x")
            assert p.is_configured() is True
...
class TestAzureInitialize:
    ...
    def test_success(self):
        ...
        with (
            patch("test_ai.providers.azure_openai_provider.AzureOpenAI", mock_cls),
            patch(
                "test_ai.providers.azure_openai_provider.AsyncAzureOpenAI",
                mock_async_cls,
            ),
        ):
            p = AzureOpenAIProvider(api_key="k", endpoint="https://x")
    ...
    def test_no_key_no_settings(self):
        ...
        settings_mod = MagicMock(
            get_settings=MagicMock(side_effect=RuntimeError("no settings"))
        )
        with (
            patch("test_ai.providers.azure_openai_provider.AzureOpenAI", mock_cls),
            patch.dict("sys.modules", {"test_ai.config.settings": settings_mod}),
...
class TestAzureComplete:
    ...
    def test_rate_limit(self):
        ...
        with patch(
            "test_ai.providers.azure_openai_provider.OpenAIRateLimitError", mock_rle
        ):
...
class TestAzureCompleteAsync:
    ...
    def test_success(self):
        ...
        result = _run(
            p.complete_async(_basic_request(max_tokens=50, stop_sequences=["Y"]))
        )
...
class TestAzureStream:
    ...
    def test_stream_success(self):
        ...
        chunks = list(
            p.complete_stream(_basic_request(max_tokens=100, stop_sequences=["X"]))
        )
        assert len(chunks) == 2
