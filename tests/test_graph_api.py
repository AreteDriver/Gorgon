...
class TestGraphExecuteEndpoint:
    ...
    def test_execute_simple_graph(self, client, auth_headers):
        ...
        with patch("test_ai.api_routes.graph.state.execution_manager", new=None):
...
class TestGraphExecuteAsyncEndpoint:
    ...
    def test_requires_auth(self, client):
        response = client.post(
            "/v1/graph/execute/async", json={"graph": _simple_graph()}
        )
        assert response.status_code == 401
    ...
    def test_async_returns_execution_id(self, client, auth_headers):
        ...
        with patch("test_ai.api_routes.graph.state.execution_manager", new=None):
            response = client.post(
                "/v1/graph/execute/async", json=body, headers=auth_headers
            )
...
class TestGraphExecutionStatusEndpoint:
    ...
    def test_get_after_async_submit(self, client, auth_headers):
        ...
        response = client.get(
            f"/v1/graph/executions/{execution_id}", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
...
class TestGraphPauseEndpoint:
    ...
    def test_not_found(self, client, auth_headers):
        response = client.post(
            "/v1/graph/executions/nonexistent/pause", headers=auth_headers
        )
        assert response.status_code == 404
    ...
    def test_pause_non_running_fails(self, client, auth_headers):
        ...
        from test_ai.api_routes.graph import _async_executions
        ...
        response = client.post(
            "/v1/graph/executions/test-done/pause", headers=auth_headers
        )
...
class TestGraphValidateEndpoint:
    ...
    def test_valid_graph(self, client, auth_headers):
        ...
        response = client.post(
            "/v1/graph/validate", json=_simple_graph(), headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_cycle_graph_invalid(self, client, auth_headers):
        ...
        response = client.post(
            "/v1/graph/validate", json=_graph_with_cycle(), headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
...
class TestGraphCLI:
    ...
    def test_execute_simple_graph(self, tmp_path):
        ...
        with patch("test_ai.cli.commands.graph.asyncio.run") as mock_run:
