...
from test_ai.skills.models import (
    ContractProvides,
    ContractRequires,
    EscalationRule,
    RoutingExclusion,
...
class TestVerificationCheckpoint:
    def test_construction(self):
        cp = VerificationCheckpoint(
            trigger="Before destructive op", action="Verify backup"
        )
...
class TestSkillVerification:
    ...
    def test_full_construction(self):
        ver = SkillVerification(
            ...
            checkpoints=[
                VerificationCheckpoint(trigger="Before delete", action="Verify backup")
            ],
...
class TestSkillErrorHandling:
    ...
    def test_full_construction(self):
        eh = SkillErrorHandling(
            escalation=[
                EscalationRule(error_class="recoverable", action="retry", max_retries=3)
            ],
    ...
    def test_v2_fields(self):
        skill = SkillDefinition(
            ...
            contracts=SkillContracts(
                ...
                requires=[
                    ContractRequires(name="target_paths", provider="context-mapper")
                ],
            ),
