...
from test_ai.plugins.installer import PluginInstaller
from test_ai.plugins.models import (
    PluginInstallation,
    PluginInstallRequest,
    PluginSource,
    PluginUpdateRequest,
)
from test_ai.plugins.registry import PluginRegistry
...
class TestInstall:
    ...
    @patch("test_ai.plugins.installer.load_plugin_from_file")
    ...
    @patch("test_ai.plugins.installer.load_plugin_from_file")
    def test_install_validation_fails(
    ...
    @patch("test_ai.plugins.installer.load_plugin_from_file")
    def test_install_validation_exception(
        self, mock_load, installer, tmp_path, mock_backend
    ):
        mock_backend.execute.return_value = []
...
class TestUninstall:
    ...
    def test_uninstall_registry_error_still_succeeds(
        self, installer, mock_backend, mock_registry
    ):
        mock_backend.execute.side_effect = [
            [_installation_row()],
            None,
...
class TestEnableDisable:
    ...
    def test_disable_registry_error_still_succeeds(
        self, installer, mock_backend, mock_registry
    ):
        mock_backend.execute.side_effect = [
            [_installation_row(enabled=True)],
...
class TestDownloadHelpers:
    ...
    def test_download_from_github_valid_url(self, installer):
        with patch.object(installer, "_download_and_verify") as mock_dl:
            ...
            result = installer._download_from_github(
    ...
    def test_download_from_url_valid(self, installer):
        with patch.object(installer, "_download_and_verify") as mock_dl:
            mock_dl.return_value = (Path("/tmp/plugin.py"), "checksum")
            result = installer._download_from_url(
                "https://example.com/plugin.py", "test"
            )
            assert result == (Path("/tmp/plugin.py"), "checksum")
...
class TestDownloadAndVerify:
    ...
    @patch("test_ai.plugins.installer.urlretrieve")
