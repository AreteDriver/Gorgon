"""Supplementary tests for test_ai.tui.commands — targets uncovered branches.
...
from test_ai.tui.commands import (
    CommandRegistry,
    _clipboard_copy,
    _is_sensitive_path,
...
class TestClipboardCopy:
    ...
    def test_darwin_pbcopy_success(self):
        with (
            patch("test_ai.tui.commands.platform") as mock_platform,
            patch("test_ai.tui.commands.subprocess") as mock_sub,
        ):
    ...
    def test_linux_xclip_success(self):
        with (
            patch("test_ai.tui.commands.platform") as mock_platform,
            patch("test_ai.tui.commands.shutil") as mock_shutil,
            patch("test_ai.tui.commands.subprocess") as mock_sub,
        ):
    ...
    def test_linux_xsel_fallback(self):
        with (
            ...
            mock_platform.system.return_value = "Linux"
            mock_shutil.which.side_effect = lambda name: (
                "/usr/bin/xsel" if name == "xsel" else None
            )
            proc = MagicMock()
            proc.returncode = 0
            mock_sub.run.return_value = proc
...
class TestCmdSave:
    def test_save_no_title(self):
        ...
        with patch("test_ai.tui.session.TUISession.save"):
...
class TestCmdLoad:
    def test_load_no_sessions(self):
        app = _make_app()
        registry = create_command_registry(app)
        with patch("test_ai.tui.session.TUISession.list_sessions", return_value=[]):
    ...
    def test_load_list_sessions(self):
        ...
        with patch(
            "test_ai.tui.session.TUISession.list_sessions",
    ...
    def test_load_by_index(self):
        ...
        with (
            ...
            patch(
                "test_ai.tui.session.TUISession.load",
...
class TestCmdCopy:
    def test_copy_success(self):
        ...
        with patch(
            "test_ai.tui.commands._clipboard_copy",
