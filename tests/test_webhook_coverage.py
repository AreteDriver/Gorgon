...
def _make_webhook_manager(backend=None):
    ...
    with (
        patch(
            "test_ai.webhooks.webhook_manager.get_settings",
            return_value=MagicMock(),
        ),
        patch(
            "test_ai.webhooks.webhook_manager.get_database",
...
def _make_delivery_manager(backend=None):
    ...
    backend = backend or _mock_backend()
    with patch(
        "test_ai.webhooks.webhook_delivery.get_database",
        return_value=backend,
    ):
...
class TestSignatureVerification:
    ...
    def test_verify_signature_with_prefix(self):
        ...
        payload = b'{"event": "push"}'
        expected_sig = (
            "sha256=" + hmac.new(b"my-secret", payload, hashlib.sha256).hexdigest()
        )
