...
class TestProcessWorkflowResult:
    ...
    def test_all_steps_succeed(self):
        outcomes = [
            _make_outcome(
                step_id="s1", agent_role="builder", success=True, cost_usd=0.01
            ),
            _make_outcome(
                step_id="s2", agent_role="reviewer", success=True, cost_usd=0.01
            ),
        ]
        engine = _make_engine()
    ...
    def test_partial_failure_generates_insight(self):
        outcomes = [
            ...
            _make_outcome(
                step_id="s2", agent_role="builder", success=True, cost_usd=0.01
            ),
            _make_outcome(
                step_id="s3", agent_role="builder", success=False, cost_usd=0.01
            ),
        ]
        engine = _make_engine()
        engine._tracker.get_workflow_outcomes.return_value = outcomes
    ...
    def test_learning_store_failure_handled(self):
        outcomes = [
            ...
            _make_outcome(
                step_id="s2", agent_role="builder", success=False, cost_usd=0.01
            ),
        ]
        engine = _make_engine()
        engine._tracker.get_workflow_outcomes.return_value = outcomes
...
class TestSuggestWorkflowImprovements:
    ...
    def test_low_success_rate_suggests_upgrade(self):
        ...
        upgrade_suggestions = [
            s for s in suggestions if s.category == "provider_upgrade"
        ]
    ...
    def test_high_cost_step_suggests_downgrade(self):
        outcomes = [
            _make_outcome(
                step_id="s1", agent_role="builder", success=True, cost_usd=0.80
            ),
            _make_outcome(
                step_id="s2", agent_role="builder", success=True, cost_usd=0.80
            ),
            _make_outcome(
                step_id="s3", agent_role="builder", success=True, cost_usd=0.80
            ),
            _make_outcome(
                step_id="s4", agent_role="reviewer", success=True, cost_usd=0.05
            ),
        ]
        engine = _make_engine()
        engine._tracker.get_workflow_outcomes.return_value = outcomes
    ...
    def test_parallelization_suggestion(self):
        ...
        many_outcomes = [
            _make_outcome(step_id=f"sb{i}", agent_role="builder", success=True)
            for i in range(5)
        ] + [
            _make_outcome(step_id=f"sr{i}", agent_role="reviewer", success=True)
            for i in range(5)
        ]
    ...
    def test_suggestions_sorted_by_impact(self):
        ...
        if len(suggestions) >= 2:
            ...
            for i in range(len(suggestions) - 1):
                assert impact_order.get(
                    suggestions[i].estimated_impact, 3
                ) <= impact_order.get(suggestions[i + 1].estimated_impact, 3)
