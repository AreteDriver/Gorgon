...
from test_ai.auth.tenants import (
    Organization,
    OrganizationInvite,
    OrganizationMember,
    OrganizationRole,
...
class TestTenantPermissions:
    ...
    def test_check_permission_viewer(self, manager, mock_backend):
        ...
        mock_backend.execute.return_value = [
            ("m1", "org1", "user1", "viewer", "2026-01-01", None)
        ]
        result = manager.check_permission("org1", "user1", OrganizationRole.VIEWER)
        assert result is True
    ...
    def test_check_permission_owner(self, manager, mock_backend):
        ...
        mock_backend.execute.return_value = [
            ("m1", "org1", "user1", "owner", "2026-01-01", None)
        ]
...
class TestTenantIntegration:
    ...
    def test_role_hierarchy(self):
        ...
        assert (
            role_hierarchy[OrganizationRole.OWNER]
            > role_hierarchy[OrganizationRole.ADMIN]
        )
        assert (
            role_hierarchy[OrganizationRole.ADMIN]
            > role_hierarchy[OrganizationRole.MEMBER]
        )
        assert (
            role_hierarchy[OrganizationRole.MEMBER]
            > role_hierarchy[OrganizationRole.VIEWER]
        )
