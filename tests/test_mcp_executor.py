"""Tests for MCP tool workflow executor (test_ai.workflow.executor_mcp)."""
...
from test_ai.workflow.loader import VALID_STEP_TYPES, StepConfig, WorkflowConfig
...
class TestMCPToolServerResolution:
    ...
    def test_resolve_by_uuid(self, live_executor):
        ...
        with patch("test_ai.state.database.get_database"):
            with patch(
                "test_ai.mcp.manager.MCPConnectorManager",
                return_value=mock_manager,
            ):
                result = live_executor._resolve_mcp_server(
...
class TestMCPToolExecution:
    ...
    def _run_step(self, executor, server, tool_result, arguments=None):
        ...
        with patch.object(executor, "_resolve_mcp_server", return_value=server):
            with patch.object(executor, "_get_mcp_auth_headers", return_value=None):
                with patch(
                    "test_ai.mcp.client.call_mcp_tool",
    ...
    def test_tool_error(self, live_executor):
        ...
        with pytest.raises(RuntimeError, match="returned error"):
            self._run_step(
                live_executor, server, {"content": "not found", "is_error": True}
            )
    ...
    def test_sdk_missing_raises(self, live_executor):
        ...
        with patch.object(live_executor, "_resolve_mcp_server", return_value=server):
            with patch.object(
                live_executor, "_get_mcp_auth_headers", return_value=None
            ):
                with patch(
                    "test_ai.mcp.client.call_mcp_tool",
    ...
    def test_passes_headers_to_client(self, live_executor):
        ...
        with patch.object(live_executor, "_resolve_mcp_server", return_value=server):
            with patch.object(
                live_executor, "_get_mcp_auth_headers", return_value=auth_headers
            ):
                with patch(
                    "test_ai.mcp.client.call_mcp_tool",
