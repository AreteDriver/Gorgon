...
- src/test_ai/dashboard/plugin_marketplace.py (168 missed lines)
- src/test_ai/cli/commands/calendar_cmd.py (168 missed lines)
- src/test_ai/cli/commands/bot.py (136 missed lines)
- src/test_ai/cli/commands/browser.py (99 missed lines)
"""
...
from test_ai.cli.main import app
...
_MARKETPLACE_MODULES = [
    "test_ai.dashboard.plugin_marketplace",
]
...
class TestRenderPluginCard:
    ...
    def test_render_plugin_card_not_verified_not_featured(self, mock_streamlit):
        ...
        mock_streamlit.session_state.installed_plugins = {}
        plugin = next(
            p for p in SAMPLE_PLUGINS if not p.get("verified") and not p.get("featured")
        )
        _render_plugin_card(plugin)
        mock_streamlit.markdown.assert_called()
...
class TestRenderPluginMarketplace:
    ...
    def test_render_marketplace_with_selected_plugin(self, mock_streamlit):
        ...
        mock_streamlit.session_state["marketplace_selected_plugin"] = (
            "github-integration"
        )
...
class TestCalendarList:
    ...
    def test_list_import_error(self):
        ...
        with patch.dict(sys.modules, {"test_ai.api_clients": None}):
    ...
    def test_list_auth_failure(self):
        ...
        with patch.dict(sys.modules, {"test_ai.api_clients": mock_module}):
            result = runner.invoke(app, ["calendar", "list"])
            assert result.exit_code != 0
...
class TestCalendarAdd:
    ...
    def test_add_auth_failure(self):
        ...
        with patch.dict(sys.modules, {"test_ai.api_clients": mock_module}):
            result = runner.invoke(
                app, ["calendar", "add", "Test", "--start", "2024-01-15 14:00"]
            )
            assert result.exit_code != 0
...
class TestBotTelegram:
    """Test bot telegram command."""

    def test_telegram_no_token(self):
        result = runner.invoke(app, ["bot", "telegram"])
        assert result.exit_code != 0
        assert (
            "token required" in result.output.lower()
            or "token" in result.output.lower()
        )

    def test_telegram_import_error(self):
        with patch.dict(sys.modules, {"test_ai.messaging": None}):
            result = runner.invoke(app, ["bot", "telegram", "--token", "123:ABC"])
            assert result.exit_code != 0

    def test_telegram_with_token_and_users(self):
        """Test telegram bot starts with token and user lists."""
        mock_messaging = MagicMock()
        mock_chat = MagicMock()
        mock_backends = MagicMock()

        mock_bot_instance = MagicMock()
        mock_bot_instance.is_running = False  # Exit loop immediately
        mock_messaging.TelegramBot.return_value = mock_bot_instance
        ...
        mock_bot_instance.start = fake_start
        mock_bot_instance.stop = fake_stop

        with patch.dict(
            sys.modules,
            {
                "test_ai.messaging": mock_messaging,
                "test_ai.chat": mock_chat,
                "test_ai.state.backends": mock_backends,
            },
        ):
            result = runner.invoke(
                app,
    ...
    def test_telegram_bot_exception(self):
        """Test telegram bot handles runtime exceptions."""
        mock_messaging = MagicMock()
        mock_chat = MagicMock()
        mock_backends = MagicMock()
        ...
        with patch.dict(
            ...
            result = runner.invoke(
                app,
                ["bot", "telegram", "--token", "123456:ABC-DEF"],
            )
            assert result.exit_code != 0


class TestBotDiscord:
    """Test bot discord command."""

    def test_discord_no_token(self):
        result = runner.invoke(app, ["bot", "discord"])
        assert result.exit_code != 0

    def test_discord_import_error(self):
        with patch.dict(sys.modules, {"test_ai.messaging": None}):
            result = runner.invoke(
                app, ["bot", "discord", "--token", "my-discord-token"]
            )
            assert result.exit_code != 0

    def test_discord_with_token_and_options(self):
        """Test discord bot starts with all options."""
        mock_messaging = MagicMock()
        mock_chat = MagicMock()
        mock_backends = MagicMock()
        ...
        mock_messaging.DiscordBot.return_value = mock_bot_instance
    ...
    def test_discord_bot_exception(self):
        """Test discord bot handles runtime exceptions."""
        mock_messaging = MagicMock()
        mock_chat = MagicMock()
        mock_backends = MagicMock()
        ...
        async def fake_start():
            raise RuntimeError("Discord connection failed")
        ...
        with patch.dict(
            ...
            result = runner.invoke(
                app,
                ["bot", "discord", "--token", "my-discord-token"],
            )
            assert result.exit_code != 0
...
class TestBotStatus:
    ...
    def test_status_with_all_configured(self):
        mock_settings = MagicMock()
        mock_settings.telegram_bot_token = "fake-token"
        mock_settings.telegram_allowed_users = "111,222"
        mock_settings.telegram_admin_users = "111"
    ...
    def test_status_with_nothing_configured(self):
        mock_settings = MagicMock()
        ...
        with patch("test_ai.config.settings.get_settings", return_value=mock_settings):
            result = runner.invoke(app, ["bot", "status"])
            assert result.exit_code == 0
            assert "Not set" in result.output or "not" in result.output.lower()

    def test_status_checks_dependencies(self):
        mock_settings = MagicMock()
        mock_settings.telegram_bot_token = None
        mock_settings.telegram_allowed_users = None
        mock_settings.telegram_admin_users = None
...
class TestBotSetup:
    """Test bot setup command."""

    def test_setup_skip_telegram(self):
        result = runner.invoke(app, ["bot", "setup"], input="n\n")
        assert result.exit_code == 0
        assert "Skipping" in result.output

    def test_setup_with_telegram_token(self):
        result = runner.invoke(
            app,
            ["bot", "setup"],
            input="y\n123456789:ABCDEFGHIJ-klmnopqrstuvwxyz12345\nn\n",
        )
        assert result.exit_code == 0
        assert "TELEGRAM_BOT_TOKEN" in result.output

    def test_setup_with_short_token_warning(self):
        result = runner.invoke(
            app,
            ["bot", "setup"],
            input="y\nshort\nn\n",
    ...
    def test_setup_with_restricted_users(self):
        result = runner.invoke(
            app,
            ["bot", "setup"],
            input="y\n123456789:ABCDEFGHIJ-klmnopqrstuvwxyz12345\ny\n111,222\n",
        ...
        assert result.exit_code == 0
        assert "TELEGRAM_ALLOWED_USERS" in result.output
...
class TestBrowserNavigate:
    ...
    def test_navigate_with_screenshot(self):
        mock_module = _make_browser_mock(
            ...
            screenshot_result=FakePageResult(
...
class TestBrowserScreenshot:
    ...
    def test_screenshot_import_error(self):
        with patch.dict(sys.modules, {"test_ai.browser": None}):
            result = runner.invoke(
                app, ["browser", "screenshot", "https://example.com"]
            )
            assert result.exit_code != 0
    ...
    def test_screenshot_with_output_path(self):
        mock_module = _make_browser_mock(
            nav_result=FakePageResult(success=True, title="Example"),
            screenshot_result=FakePageResult(
                success=True, screenshot_path="/tmp/custom.png"
            ),
        )
        with patch.dict(sys.modules, {"test_ai.browser": mock_module}):
            result = runner.invoke(
...
class TestBrowserExtract:
    ...
    def test_extract_success_json(self):
        ...
        with patch.dict(sys.modules, {"test_ai.browser": mock_module}):
            result = runner.invoke(
                app, ["browser", "extract", "https://example.com", "--json"]
            )
            assert result.exit_code == 0
            assert "Example" in result.output
