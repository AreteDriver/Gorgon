...
class TestDoTaskLiveFlag:
    ...
    @patch("test_ai.cli.commands.dev.format_context_for_prompt", return_value="ctx")
    ...
    @patch("test_ai.cli.commands.dev.detect_codebase_context")
    @patch("test_ai.workflow.loader.load_workflow")
    def test_live_creates_execution_manager(
        self, mock_load, mock_ctx, mock_fmt, mock_exec
    ):
        ...
        with patch.object(Path, "exists", return_value=True):
            with patch(
                "test_ai.cli.helpers._create_cli_execution_manager"
            ) as mock_create_em:
                mock_em = MagicMock()
    ...
    @patch("test_ai.cli.commands.dev.detect_codebase_context")
    @patch("test_ai.workflow.loader.load_workflow")
    def test_without_live_uses_status_spinner(
        self, mock_load, mock_ctx, mock_fmt, mock_exec
    ):
...
class TestCreateCliExecutionManager:
    ...
    def test_returns_none_on_failure(self):
        ...
        with patch(
            "test_ai.state.backends.SQLiteBackend",
