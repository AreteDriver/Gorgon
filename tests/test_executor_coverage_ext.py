...
from test_ai.workflow.executor_results import ExecutionResult, StepResult, StepStatus
from test_ai.workflow.loader import StepConfig, WorkflowConfig, WorkflowSettings
...
class TestExecuteShellExtended:
    ...
    def test_missing_command_raises(self):
        ...
        with patch("test_ai.config.get_settings") as mock_settings:
    ...
    @patch("test_ai.workflow.executor_integrations.subprocess.run")
    @patch("test_ai.config.get_settings")
    @patch("test_ai.utils.validation.validate_shell_command")
    @patch("test_ai.utils.validation.substitute_shell_variables")
    def test_allowed_commands_whitelist_passes(
        self, mock_sub, mock_val, mock_settings, mock_run
    ):
        """Command in whitelist is allowed."""
        mock_settings.return_value = self._settings_mock(
            shell_allowed_commands="echo,ls,cat"
        )
    ...
    def test_allowed_commands_whitelist_blocks(self, mock_settings):
        ...
        mock_settings.return_value = self._settings_mock(
            shell_allowed_commands="echo,ls"
        )
        exe = _bare_executor()
    ...
    @patch("test_ai.utils.validation.validate_shell_command")
    @patch("test_ai.utils.validation.substitute_shell_variables")
    def test_allow_failure_non_zero_exit(
        self, mock_sub, mock_val, mock_settings, mock_run
    ):
    ...
    @patch("test_ai.utils.validation.validate_shell_command")
    @patch("test_ai.utils.validation.substitute_shell_variables")
    def test_step_level_timeout_override(
        self, mock_sub, mock_val, mock_settings, mock_run
    ):
    ...
    @patch("test_ai.utils.validation.validate_shell_command")
    @patch("test_ai.utils.validation.substitute_shell_variables")
    def test_timeout_expired_with_partial_output(
        self, mock_sub, mock_val, mock_settings, mock_run
    ):
...
class TestExecuteGitHubExtended:
    ...
    @patch("test_ai.api_clients.GitHubClient")
...
class TestExecuteNotion:
    ...
    def test_dry_run(self):
        exe = _bare_executor(dry_run=True)
        step = _make_step(
            step_type="notion", params={"action": "search", "query": "test"}
        )
        out = exe._execute_notion(step, {})
        assert out["dry_run"] is True
        assert "DRY RUN" in out["result"]

    @patch("test_ai.api_clients.NotionClientWrapper")
...
class TestExecuteGmail:
    ...
    @patch("test_ai.api_clients.GmailClient")
...
class TestExecuteSlack:
    ...
    @patch("test_ai.api_clients.slack_client.SlackClient")
    @patch("test_ai.config.get_settings")
    ...
    @patch("test_ai.api_clients.slack_client.SlackClient")
    @patch("test_ai.api_clients.slack_client.MessageType")
    @patch("test_ai.config.get_settings")
    ...
    def test_unknown_action_raises(self, mock_settings, MockSlack):
        ...
        exe = _bare_executor(dry_run=False)
        step = _make_step(
            step_type="slack", params={"action": "delete_channel", "channel": "#dev"}
        )
...
class TestExecuteCalendar:
    ...
    @patch("test_ai.api_clients.calendar_client.CalendarClient")
    ...
    def test_list_events(self, MockCal):
        ...
        mock_event.start = datetime(2025, 1, 1, 9, 0, tzinfo=timezone.utc)
        mock_event.end = datetime(2025, 1, 1, 9, 30, tzinfo=timezone.utc)
    ...
    @patch("test_ai.api_clients.calendar_client.CalendarEvent")
    @patch("test_ai.api_clients.calendar_client.CalendarClient")
    ...
    def test_get_event(self, MockCal):
        ...
        result_event.start = datetime(2025, 3, 1, 14, 0, tzinfo=timezone.utc)
        result_event.end = datetime(2025, 3, 1, 15, 0, tzinfo=timezone.utc)
...
class TestExecuteBrowser:
    ...
    def test_screenshot(self, MockConfig, MockBrowser):
        ...
        ss_result = MagicMock(
            screenshot_path="/tmp/screenshot.png", success=True, error=None
        )
    ...
    def test_extract(self, MockConfig, MockBrowser):
        ...
        browser_instance.extract_content.assert_called_once_with(
            ".content", True, False
        )
    ...
    def test_wait(self, MockConfig, MockBrowser):
        ...
        browser_instance.wait_for_selector.assert_called_once_with(
            "#loaded", "visible", 5000
        )
...
class TestExecuteParallelGroup:
    ...
    @patch("test_ai.workflow.executor_parallel_exec.get_parallel_tracker")
    @patch("test_ai.workflow.executor_parallel_exec.ParallelExecutor")
    ...
    @patch("test_ai.workflow.executor_parallel_exec.get_parallel_tracker")
    @patch("test_ai.workflow.executor_parallel_exec.RateLimitedParallelExecutor")
    ...
    def test_successful_execution_completes_tracking(self, MockPE, mock_tracker):
        ...
        success_result = StepResult(
            step_id="s1", status=StepStatus.SUCCESS, output={"data": "ok"}
        )
...
class TestExecuteParallelGroupAsync:
    ...
    def test_async_parallel_success(self):
        ...
        asyncio.run(
            exe._execute_parallel_group_async(steps, "wf-1", result, max_workers=2)
        )
...
class TestExecuteWithAutoParallel:
    ...
    @patch("test_ai.workflow.executor_parallel_exec.find_parallel_groups")
    @patch("test_ai.workflow.executor_parallel_exec.build_dependency_graph")
    ...
    def test_single_step_failure_continue_stores(self, mock_build, mock_find):
        ...
        failed_result = StepResult(
            step_id="s1", status=StepStatus.FAILED, error="non-fatal"
        )
...
class TestParallelGroupHandlerClosure:
    ...
    def test_handler_success_calls_complete_branch(self, MockPE, mock_tracker):
        ...
        success_result = StepResult(
            step_id="s1", status=StepStatus.SUCCESS, tokens_used=42
        )
    ...
    def test_handler_failed_step_calls_fail_branch(self, MockPE, mock_tracker):
        ...
        failed_result = StepResult(
            step_id="s1", status=StepStatus.FAILED, error="step failed"
        )
    ...
    def test_handler_none_result_zero_tokens(self, MockPE, mock_tracker):
        ...
        success_result = StepResult(
            step_id="s1", status=StepStatus.SUCCESS, tokens_used=0
        )
