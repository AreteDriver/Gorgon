...
class TestInitCommand:
    ...
    def test_init_overwrites_with_confirm(self, tmp_path):
        ...
        result = runner.invoke(
            app, ["init", "Test", "-o", str(output_file)], input="y\n"
        )
...
class TestRunCommand:
    ...
    def test_run_with_variables(self, mock_get_engine):
        ...
        result = runner.invoke(
            app, ["run", "test", "--var", "key1=value1", "--var", "key2=value2"]
        )
...
class TestBudgetSubcommands:
    ...
    @patch("test_ai.budget.BudgetManager")
...
class TestScheduleSubcommands:
    ...
    @patch("test_ai.workflow.WorkflowScheduler")
    ...
    @patch("test_ai.workflow.WorkflowScheduler")
    @patch("test_ai.workflow.ScheduleConfig")
    def test_schedule_add_with_cron(self, mock_config, mock_scheduler_class):
        ...
        result = runner.invoke(
            app, ["schedule", "add", "workflow.json", "--cron", "0 * * * *"]
        )
...
class TestMemorySubcommands:
    ...
    @patch("test_ai.state.AgentMemory")
