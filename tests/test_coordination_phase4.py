    ...
    def test_no_cycles_returns_empty(self):
        ...
        backend.publish(
            Intent(
                ...
                provides=[
                    InterfaceSpec(
                        name="Auth", kind=InterfaceKind.FUNCTION, signature=""
                    )
                ],
    ...
    def test_detects_cycles(self):
        ...
        backend.publish(
            Intent(
                ...
                provides=[
                    InterfaceSpec(
                        name="AService", kind=InterfaceKind.FUNCTION, signature=""
                    )
                ],
                requires=[
                    InterfaceSpec(
                        name="BService", kind=InterfaceKind.FUNCTION, signature=""
                    )
                ],
                evidence=[Evidence(kind=EvidenceKind.TEST_PASS, description="ok")],
            )
    ...
    def test_returns_ordered_list(self):
        ...
        backend.publish(
            Intent(
                ...
                provides=[
                    InterfaceSpec(
                        name="DBService", kind=InterfaceKind.FUNCTION, signature=""
                    )
                ],
                evidence=[Evidence(kind=EvidenceKind.TEST_PASS, description="ok")],
            )
        ...
        backend.publish(
            Intent(
                ...
                provides=[
                    InterfaceSpec(name="API", kind=InterfaceKind.FUNCTION, signature="")
                ],
                evidence=[Evidence(kind=EvidenceKind.TEST_PASS, description="ok")],
            )
        )
...
class TestCoordinationAPI:
    ...
    def test_health_no_convergent(self, monkeypatch):
        from test_ai.api_routes.coordination import coordination_health

        monkeypatch.setattr(
            "test_ai.api_routes.coordination.state",
            MagicMock(coordination_bridge=None),
        )
        with patch("test_ai.agents.convergence.HAS_CONVERGENT", False):
    ...
    def test_health_no_bridge(self, monkeypatch):
        ...
        monkeypatch.setattr("test_ai.api_routes.coordination.state", mock_state)
    ...
    def test_events_no_convergent(self, monkeypatch):
        from test_ai.api_routes.coordination import coordination_events
    ...
    def test_events_invalid_type(self, monkeypatch):
        ...
        result = coordination_events(
            event_type="nonexistent_type", agent=None, limit=50
        )
...
class TestCoordinationCLI:
    ...
    def test_health_no_convergent(self, monkeypatch):
        ...
        from test_ai.cli.commands.coordination import coordination_app

        monkeypatch.setattr("test_ai.agents.convergence.HAS_CONVERGENT", False)
        runner = CliRunner()
