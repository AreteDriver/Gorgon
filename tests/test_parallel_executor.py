...
class TestParallelTask:
    ...
    def test_is_ready_with_deps(self):
        task = ParallelTask(
            id="t1", step_id="s1", handler=_ok_handler, dependencies=["dep1"]
        )
...
class TestExecuteThreaded:
    ...
    def test_fail_fast(self):
        ...
        tasks = [
            ...
            ParallelTask(
                id="t2", step_id="s2", handler=_ok_handler, dependencies=["t1"]
            ),
        ]
...
class TestExecuteStepsParallel:
    ...
    def test_with_strategy(self):
        ...
        result = execute_steps_parallel(
            steps, handler, strategy=ParallelStrategy.THREADING
        )
        assert result.all_succeeded
