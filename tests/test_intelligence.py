...
from test_ai.intelligence.outcome_tracker import OutcomeRecord, OutcomeTracker
...
class TestOutcomeTracker:
    ...
    def test_get_provider_stats(self, tracker):
        ...
        tracker.record(
            _make_outcome(step_id="s1", cost_usd=0.01, latency_ms=100, success=True)
        )
        tracker.record(
            _make_outcome(step_id="s2", cost_usd=0.03, latency_ms=300, success=False)
        )
...
class TestCrossWorkflowMemory:
    ...
    def test_build_context_for_agent(self, cross_memory, memory):
        ...
        cross_memory.record_learning(
            "builder", "FastAPI uses async endpoints", tags=["tip"]
        )
        cross_memory.record_learning(
            "builder", "Always validate input data", tags=["tip"]
        )
        ...
        context = cross_memory.build_context_for_agent(
            "builder", "Write a new FastAPI endpoint"
        )
    ...
    def test_detect_patterns(self, cross_memory):
        ...
        for i in range(4):
            cross_memory.record_learning(
                "tester", f"Always check error handling in test {i}"
            )
        ...
        assert any(
            "error" in p or "handling" in p or "check" in p for p in phrase_texts
        )
    ...
    def test_promote_memory(self, cross_memory, memory):
        ...
        mid = cross_memory.record_learning(
            "builder", "Important insight", importance=0.3
        )
        result = cross_memory.promote_memory(mid, 0.9)
        assert result is True

        # Verify the importance changed by checking the raw DB value
        row = memory.backend.fetchone(
            "SELECT importance FROM agent_memories WHERE id = ?", (mid,)
        )
...
class TestProviderRouter:
    ...
    def test_fallback_set_for_critical_complexity(self):
        ...
        sel = router.select_provider(
            "builder", task_complexity="critical", strategy="fallback"
        )
        assert sel.fallback is not None
...
class TestFeedbackEngine:
    ...
    def test_process_workflow_result_produces_insights(self, engine, tracker):
        ...
        tracker.record(
            _make_outcome(step_id="s1", workflow_id="wf1", success=True, cost_usd=0.05)
        )
        tracker.record(
            _make_outcome(step_id="s2", workflow_id="wf1", success=False, cost_usd=0.01)
        )
