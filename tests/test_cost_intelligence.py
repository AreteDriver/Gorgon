...
from test_ai.metrics.cost_tracker import CostEntry, CostTracker, Provider, TokenUsage
...
class TestAnalyzeSpending:
    ...
    def test_multiple_providers(self):
        entries = [
            _make_entry(
                days_ago=1, provider=Provider.OPENAI, model="gpt-4o", cost_usd=5.0
            ),
            _make_entry(
                days_ago=1,
    ...
    def test_top_roles_capped_at_3(self):
        entries = [
            _make_entry(days_ago=1, agent_role=f"role-{i}", cost_usd=float(i))
            for i in range(5)
        ]
        tracker = _make_tracker(entries)
        ci = CostIntelligence(tracker)
...
class TestRecommendSavings:
    ...
    def test_recommendations_sorted_by_savings(self):
        ...
        if len(recs) >= 2:
            assert (
                recs[0].estimated_savings_monthly >= recs[1].estimated_savings_monthly
            )
...
class TestGetROIReport:
    ...
    def test_roi_efficiency_score_scales(self):
        ...
        cheap_entries = [
            _make_entry(
                model="gpt-4o-mini", cost_usd=0.001, input_tokens=100, output_tokens=50
            )
            for _ in range(10)
        ]
    ...
    def test_roi_with_outcome_tracker(self):
        entries = [
            _make_entry(
                model="gpt-4o", cost_usd=0.02, input_tokens=1000, output_tokens=500
            )
            for _ in range(10)
        ]
        tracker = _make_tracker(entries)
...
class TestInternalHelpers:
    ...
    def test_pricing_ratio_from_zero(self):
        ...
        with patch.object(
            CostTracker, "PRICING", {"zero": {"input": 0.0, "output": 0.0}}
        ):
    ...
    def test_recommendation_confidence_medium(self):
        ...
        ci = CostIntelligence(tracker, outcome_tracker=mock_outcome)
        assert (
            ci._recommendation_confidence("builder", "gpt-4", "gpt-4o", 30) == "medium"
        )
...
class TestComputeEnforcementSavings:
    ...
    def test_enforcement_savings_multiple_groups(self):
        entries = [
            ...
            _make_entry(
                model="claude-3-sonnet", cost_usd=0.03, provider=Provider.ANTHROPIC
            ),
        ]
        tracker = _make_tracker(entries)
