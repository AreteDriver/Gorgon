...
from test_ai.workflow.approval_store import (
    ResumeTokenStore,
    get_approval_store,
...
class TestGetByToken:
    ...
    def test_expired_token_returns_none(self, store):
        ...
        row = store.backend.fetchone(
            "SELECT status FROM approval_tokens WHERE token = ?", (token,)
        )
...
class TestSingleton:
    ...
    def test_singleton_returns_same_instance(self):
        ...
        reset_approval_store()
        with patch("test_ai.state.database.get_database") as mock_db:
