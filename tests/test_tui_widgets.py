...
Targets:
  - src/test_ai/tui/widgets/chat_display.py (26% → 90%+)
  - src/test_ai/tui/widgets/sidebar.py (41% → 90%+)
  - src/test_ai/tui/streaming.py (64% → 90%+)
...
from test_ai.tui.streaming import (
    StreamResult,
    _stream_anthropic,
    _stream_fallback,
    _stream_ollama,
...
class TestChatDisplayRebuildDisplay:
    ...
    def test_rebuilds_agent_messages(self):
        cd = _make_chat_display()
        cd._message_buffer = [
            {"role": "agent", "content": "planning...", "agent": "planner"}
        ]
        cd._rebuild_display()
        assert cd.write.call_count == 2
        label_arg = cd.write.call_args_list[0][0][0]
...
class TestStreamOllamaDirect:
    ...
    def test_yields_content_chunks(self):
        async def _run():
            ...
            async def mock_stream(request):
                ...
                yield StreamChunk(
                    content="world", model="llama", provider="ollama", is_final=True
                )
    ...
    def test_captures_usage_from_chunk(self):
        async def _run():
            ...
            chunk_with_usage = StreamChunk(
                content="done", model="llama", provider="ollama"
            )
...
class TestStreamCompletionIntegration:
    ...
    def test_initialize_exception_is_swallowed(self):
        ...
        async def _run():
            ...
            with patch("test_ai.tui.streaming._stream_anthropic") as mock_stream:
    ...
    def test_provider_error_not_caught(self):
        ...
        async def _run():
            ...
            with patch(
                "test_ai.tui.streaming._stream_openai",
    ...
    def test_generic_error_triggers_fallback(self):
        ...
        async def _run():
            ...
            with (
                ...
                patch("test_ai.tui.streaming._stream_fallback") as mock_fb,
            ):
    ...
    def test_ollama_error_triggers_fallback(self):
        ...
        async def _run():
            ...
            with (
                patch(
                    "test_ai.tui.streaming._stream_ollama",
