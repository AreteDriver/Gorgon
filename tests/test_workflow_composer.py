...
from test_ai.workflow.composer import (
    SubWorkflowResult,
    WorkflowComposer,
    _resolve_value,
    _now_ms,
...
from test_ai.workflow.executor import ExecutionResult, StepStatus
...
class TestExecuteSubWorkflow:
    ...
    @patch("test_ai.workflow.composer.load_workflow")
    def test_basic_execution(self, mock_load):
        ...
        with patch("test_ai.workflow.composer.WorkflowExecutor") as MockExec:
            MockExec.return_value = mock_executor
            result = composer.execute_sub_workflow(
                step, {"parent_var": "value"}, depth=1
            )
    ...
    def test_inherits_parent_managers(self, mock_load):
        ...
        with patch("test_ai.workflow.composer.WorkflowExecutor") as MockExec:
            ...
            composer = WorkflowComposer()
            step = _make_step()
            composer.execute_sub_workflow(
                step, {}, depth=1, parent_executor=mock_parent
            )

            # Verify WorkflowExecutor was constructed with parent's managers
            constructor_kwargs = MockExec.call_args.kwargs
            assert (
                constructor_kwargs["checkpoint_manager"]
                is mock_parent.checkpoint_manager
            )
...
class TestResolveWorkflowGraph:
    ...
    def test_missing_workflow_handled(self, mock_load):
        ...
        root_wf = WorkflowConfig(
            name="root", version="1.0", description="", steps=[step]
        )
    ...
    def test_visited_dedup(self, mock_load):
        ...
        wf_a = WorkflowConfig(
            name="A", version="1.0", description="", steps=[step_b, step_c]
        )
        wf_b = WorkflowConfig(
            name="B", version="1.0", description="", steps=[step_shared_from_b]
        )
        wf_c = WorkflowConfig(
            name="C", version="1.0", description="", steps=[step_shared_from_c]
        )
