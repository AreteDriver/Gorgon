...
from test_ai.messaging.base import (
    BotMessage,
    BotUser,
    MessagePlatform,
    MessagingBot,
...
class _StubBot(MessagingBot):
    ...
    async def send_message(
        self, chat_id, content, reply_to_id=None, **kwargs
    ) -> str | None:
...
class TestMessageHandler:
    ...
    def mock_session_manager(self):
        ...
        session.mode = SimpleNamespace(value="assistant")
        session.created_at = datetime(2026, 2, 12, 10, 0)
        mgr.create_session.return_value = session
        mgr.get_session.return_value = session
        mgr.get_messages.return_value = []
        mgr.get_message_count.return_value = 5
        return mgr
    ...
    def mock_supervisor(self):
        sup = AsyncMock()
        sup.name = "Supervisor"
        sup.process.return_value = {"response": "Agent says hello"}
        return sup
    ...
    @pytest.fixture
    def handler_no_supervisor(self, mock_session_manager):
        from test_ai.messaging.handler import MessageHandler

        return MessageHandler(session_manager=mock_session_manager, supervisor=None)

    # -- _get_or_create_session --

    def test_get_or_create_session_creates_new(self, handler, mock_session_manager):
        from test_ai.messaging.handler import MessageHandler

        mock_session_manager.get_session.return_value = None
        h = MessageHandler(session_manager=mock_session_manager, supervisor=None)
        user = _make_user()

        with patch("test_ai.chat.models.ChatMode") as mock_mode:
            mock_mode.ASSISTANT = "assistant"
            sid = h._get_or_create_session(user, MessagePlatform.TELEGRAM)

        assert sid == "sess-1"
        mock_session_manager.create_session.assert_called_once()

    def test_get_or_create_session_returns_cached(self, handler):
        user = _make_user()
        handler._user_sessions[user.identifier] = "cached-sess"
        handler.session_manager.get_session.return_value = MagicMock(status="active")
        with patch("test_ai.chat.models.ChatMode"):
            sid = handler._get_or_create_session(user, MessagePlatform.TELEGRAM)
        assert sid == "cached-sess"

    def test_get_or_create_session_stale_cache(self, handler, mock_session_manager):
        user = _make_user()
        handler._user_sessions[user.identifier] = "old-sess"
        ...
        assert sid == "sess-1"
        assert handler._user_sessions[user.identifier] == "sess-1"

    def test_get_or_create_session_inactive_cache(self, handler, mock_session_manager):
        user = _make_user()
        handler._user_sessions[user.identifier] = "inactive-sess"
        inactive = MagicMock(status="closed")
        mock_session_manager.get_session.return_value = inactive
    ...
    def test_get_or_create_session_with_default_mode(self, mock_session_manager):
        from test_ai.messaging.handler import MessageHandler
        ...
        user = _make_user()
        with patch("test_ai.chat.models.ChatMode"):
            h._get_or_create_session(user, MessagePlatform.TELEGRAM)
        _, kwargs = mock_session_manager.create_session.call_args
        assert kwargs["mode"] is custom_mode

    def test_get_or_create_session_title_uses_display_name(self, mock_session_manager):
        from test_ai.messaging.handler import MessageHandler

        h = MessageHandler(session_manager=mock_session_manager)
        user = _make_user(display_name="Alice", username="alice")
        with patch("test_ai.chat.models.ChatMode") as mock_mode:
            mock_mode.ASSISTANT = "assistant"
            mock_session_manager.get_session.return_value = None
            h._get_or_create_session(user, MessagePlatform.DISCORD)
        _, kwargs = mock_session_manager.create_session.call_args
        assert "Alice" in kwargs["title"]
        assert "Discord" in kwargs["title"]

    def test_get_or_create_session_title_uses_username(self, mock_session_manager):
        from test_ai.messaging.handler import MessageHandler

        h = MessageHandler(session_manager=mock_session_manager)
        user = _make_user(display_name=None, username="bob")
        with patch("test_ai.chat.models.ChatMode") as mock_mode:
            mock_mode.ASSISTANT = "assistant"
            mock_session_manager.get_session.return_value = None
            h._get_or_create_session(user, MessagePlatform.TELEGRAM)
    ...
    def test_get_or_create_session_title_uses_id(self, mock_session_manager):
        from test_ai.messaging.handler import MessageHandler

        h = MessageHandler(session_manager=mock_session_manager)
        user = _make_user(display_name=None, username=None, id="uid-999")
        with patch("test_ai.chat.models.ChatMode") as mock_mode:
            mock_mode.ASSISTANT = "assistant"
            mock_session_manager.get_session.return_value = None
            h._get_or_create_session(user, MessagePlatform.TELEGRAM)
        _, kwargs = mock_session_manager.create_session.call_args
        assert "uid-999" in kwargs["title"]
    ...
    def test_handle_message_with_supervisor(
        ...
        async def _test():
            ...
            assert resp == "Agent says hello"
            mock_supervisor.process.assert_awaited_once()
            # Check message stored twice (user + assistant)
    ...
    def test_handle_message_without_supervisor(
        ...
        async def _test():
            ...
            assert "no agent is configured" in resp.lower()
            # Should still store user message + assistant response
            assert mock_session_manager.add_message.call_count == 2
    ...
    def test_handle_message_supervisor_error(
        ...
        async def _test():
            ...
            assert "agent crash" in resp

        asyncio.run(_test())

    def test_handle_message_stores_metadata(self, handler, mock_session_manager):
        async def _test():
            msg = _make_message(
            ...
            msg.user = _make_user(id="u-77")
            await handler.handle_message(msg)
            call_args = mock_session_manager.add_message.call_args_list[0]
            meta = call_args[1]["metadata"]
            assert meta["platform"] == "discord"
            assert meta["platform_user_id"] == "u-77"
    ...
    def test_handle_message_stores_response_none_skips(
        ...
        async def _test():
            ...
            resp = await handler.handle_message(msg)
            assert resp == ""
            # User message stored, but no assistant (empty string is falsy? Check the code: if response: ...)
            # Actually the handler stores "if response:" which is falsy for ""
            # User message is always stored, response only if truthy
    ...
    def test_process_with_supervisor_builds_context(
        ...
        async def _test():
            ...
            msg_mock.content = "history msg"
            mock_session_manager.get_messages.return_value = [msg_mock, msg_mock]

            msg = _make_message(
                content="current",
                user=_make_user(id="u1", display_name="Alice", is_admin=True),
            )
            msg.user.is_admin = True

            result = await handler._process_with_supervisor("sess-1", msg)
            assert result == "Agent says hello"

            call_kwargs = mock_supervisor.process.call_args[1]
            assert call_kwargs["message"] == "current"
            assert call_kwargs["context"]["platform"] == "telegram"
            assert call_kwargs["context"]["user_id"] == "u1"
            assert call_kwargs["context"]["is_admin"] is True
            # History should exclude last message
            assert len(call_kwargs["history"]) == 1

        asyncio.run(_test())

    def test_process_with_supervisor_no_response_key(self, handler, mock_supervisor):
        async def _test():
    ...
    # -- get_session_for_user / clear_session --

    def test_get_session_for_user_exists(self, handler):
        user = _make_user()
        handler._user_sessions[user.identifier] = "sess-abc"
        assert handler.get_session_for_user(user) == "sess-abc"

    def test_get_session_for_user_not_exists(self, handler):
        user = _make_user(id="unknown")
        assert handler.get_session_for_user(user) is None

    def test_clear_session_exists(self, handler):
        user = _make_user()
        handler._user_sessions[user.identifier] = "sess-abc"
        result = handler.clear_session(user)
        assert result is True
        assert user.identifier not in handler._user_sessions
    ...
    def test_handle_command_start(self, handler, mock_supervisor):
        async def _test():
            msg = _make_message()
            resp = await handler.handle_command(msg, "start", [])
            assert "Hello" in resp
            assert mock_supervisor.name in resp

        asyncio.run(_test())

    def test_handle_command_start_no_supervisor(self, handler_no_supervisor):
        async def _test():
            msg = _make_message()
            resp = await handler_no_supervisor.handle_command(msg, "start", [])
            assert "Gorgon" in resp

        asyncio.run(_test())

    def test_handle_command_help(self, handler):
        async def _test():
            msg = _make_message()
            resp = await handler.handle_command(msg, "help", [])
    ...
    def test_handle_command_new(self, handler):
        async def _test():
            user = _make_user()
            handler._user_sessions[user.identifier] = "old-sess"
            msg = _make_message(user=user)
            resp = await handler.handle_command(msg, "new", [])
            assert "new conversation" in resp.lower()
            assert user.identifier not in handler._user_sessions

        asyncio.run(_test())

    def test_handle_command_status_no_session(self, handler):
        async def _test():
            msg = _make_message(user=_make_user(id="no-sess"))
            resp = await handler.handle_command(msg, "status", [])
            assert "No active session" in resp

        asyncio.run(_test())

    def test_handle_command_status_with_session(self, handler, mock_session_manager):
        async def _test():
            user = _make_user()
            handler._user_sessions[user.identifier] = "sess-1"
            msg = _make_message(user=user)
            resp = await handler.handle_command(msg, "status", [])
            assert "sess-1" in resp
            assert "Messages: 5" in resp
    ...
    def test_handle_command_history_no_session(self, handler):
        async def _test():
            msg = _make_message(user=_make_user(id="no-sess"))
    ...
    def test_handle_command_history_empty(self, handler, mock_session_manager):
        async def _test():
            user = _make_user()
            handler._user_sessions[user.identifier] = "sess-1"
    ...
    def test_handle_command_history_with_messages(self, handler, mock_session_manager):
        async def _test():
            user = _make_user()
            handler._user_sessions[user.identifier] = "sess-1"
            msg1 = MagicMock()
            msg1.role.value = "user"
            msg1.content = "user msg"
            msg2 = MagicMock()
            msg2.role.value = "assistant"
            msg2.content = "bot msg"
            mock_session_manager.get_messages.return_value = [msg1, msg2]
            msg = _make_message(user=user)
            resp = await handler.handle_command(msg, "history", [])
            assert "[You]" in resp
            assert "[Gorgon]" in resp
            assert "Last 2 messages" in resp

        asyncio.run(_test())

    def test_handle_command_history_with_limit_arg(self, handler, mock_session_manager):
        async def _test():
            user = _make_user()
            handler._user_sessions[user.identifier] = "sess-1"
            mock_session_manager.get_messages.return_value = []
            msg = _make_message(user=user)
            await handler.handle_command(msg, "history", ["10"])
            mock_session_manager.get_messages.assert_called_with("sess-1", limit=10)
    ...
    def test_handle_command_history_limit_capped_at_20(
        ...
        async def _test():
            ...
            mock_session_manager.get_messages.return_value = []
            msg = _make_message(user=user)
            await handler.handle_command(msg, "history", ["50"])
            mock_session_manager.get_messages.assert_called_with("sess-1", limit=20)
    ...
    def test_handle_command_history_non_numeric_limit(
        ...
        async def _test():
            ...
            mock_session_manager.get_messages.return_value = []
            msg = _make_message(user=user)
            await handler.handle_command(msg, "history", ["abc"])
            # Default limit=5 when non-numeric
            mock_session_manager.get_messages.assert_called_with("sess-1", limit=5)
    ...
    def test_handle_command_history_truncates_long_messages(
        ...
        async def _test():
            ...
            msg1.role.value = "user"
            msg1.content = "x" * 200
            mock_session_manager.get_messages.return_value = [msg1]
            msg = _make_message(user=user)
            resp = await handler.handle_command(msg, "history", [])
            assert "..." in resp
    ...
    def test_handle_command_unknown(self, handler):
        async def _test():
            msg = _make_message()
            resp = await handler.handle_command(msg, "nonexistent", [])
            assert resp is None
