...
from test_ai.workflow.executor import WorkflowExecutor, reset_circuit_breakers
from test_ai.workflow.loader import StepConfig, WorkflowConfig, load_workflow
from test_ai.workflow.graph_models import (
    GraphEdge,
    GraphNode,
    NodePosition,
    WorkflowGraph,
)
from test_ai.workflow.graph_executor import (
    NodeStatus,
    ReactFlowExecutor,
)
...
class TestMCPToolPipeline:
    ...
    def test_mcp_tool_step_type_recognized(self):
        ...
        from test_ai.workflow.loader import VALID_STEP_TYPES
    ...
    def test_mcp_tool_variable_substitution(self):
        ...
        result = executor.execute(
            wf, inputs={"target_server": "my-db", "search_term": "hello"}
        )
        assert result.status == "success"
    ...
    @patch("test_ai.workflow.executor_mcp.MCPHandlersMixin._resolve_mcp_server")
    @patch("test_ai.mcp.client.call_mcp_tool")
...
class TestGraphExecutorPipeline:
    ...
    def test_branch_node_execution(self):
        ...
        executor = ReactFlowExecutor()
        result = asyncio.run(
            executor.execute_async(graph, variables={"run_path": "yes"})
        )
        assert result.status == "completed"
    ...
    def test_cycle_detection_fails_non_loop(self):
        ...
        graph = WorkflowGraph(
            id="cycle-test", name="Cycle Test", nodes=nodes, edges=edges
        )
        executor = ReactFlowExecutor()
        result = asyncio.run(executor.execute_async(graph))
        assert result.status == "failed"
    ...
    def test_callbacks_fire(self):
        ...
        graph = _make_simple_graph(
            agent_data={"step_type": "shell", "action": "echo test"}
        )
        executor = ReactFlowExecutor(
    ...
    def test_variables_propagate_through_nodes(self):
        ...
        graph = _make_simple_graph(
            agent_data={"step_type": "shell", "action": "echo ${greeting}"}
        )
...
class TestWebhookDeliveryPipeline:
    ...
    def test_retry_strategy_exponential_backoff(self):
        ...
        from test_ai.webhooks.webhook_delivery import RetryStrategy

        strategy = RetryStrategy(
            max_retries=5, base_delay=1.0, max_delay=30.0, jitter=False
        )
...
class TestCrossFeatureIntegration:
    ...
    def test_graph_with_variables(self):
        ...
        executor = ReactFlowExecutor()
        result = asyncio.run(
            executor.execute_async(graph, variables={"project": "gorgon"})
        )
        assert result.status == "completed"
