...
from test_ai.browser.automation import (
    ActionType,
    BrowserAutomation,
    BrowserConfig,
    PageAction,
...
def _mock_playwright_available():
    ...
    return patch("test_ai.browser.automation.PLAYWRIGHT_AVAILABLE", True)
...
def _make_browser(config=None, tmp_path=None):
    ...
    if config is None:
        config = BrowserConfig(
            ...
            if tmp_path
            else "/tmp/gorgon_test_screenshots"
        )
...
class TestBrowserAutomationInit:
    ...
    def test_init_without_playwright_raises(self):
        with patch("test_ai.browser.automation.PLAYWRIGHT_AVAILABLE", False):
...
class TestBrowserAutomationStart:
    ...
    def test_start_chromium(self, tmp_path):
        async def _test():
            ...
            mock_context = AsyncMock()
            mock_browser_type.launch.return_value.new_context = AsyncMock(
            ...
            with patch(
                "test_ai.browser.automation.async_playwright",
    ...
    def test_start_firefox(self, tmp_path):
        async def _test():
            config = BrowserConfig(
                browser_type="firefox", screenshots_dir=str(tmp_path)
            )
            browser = _make_browser(config=config)
            mock_pw_instance = AsyncMock()
            mock_browser_type = AsyncMock()
    ...
    def test_stop(self, tmp_path):
        async def _test():
            browser = _make_browser(tmp_path=tmp_path)
            mock_page, mock_context, mock_browser_obj, mock_playwright = (
...
class TestClick:
    ...
    def test_click_success(self, tmp_path):
        async def _test():
            ...
            mock_page.click.assert_awaited_once_with(
                "#submit", button="left", click_count=1
            )
    ...
    def test_click_right_button(self, tmp_path):
        async def _test():
            ...
            assert result.success is True
            mock_page.click.assert_awaited_once_with(
                "#menu", button="right", click_count=1
            )
    ...
    def test_click_double(self, tmp_path):
        async def _test():
            ...
            assert result.success is True
            mock_page.click.assert_awaited_once_with(
                "#item", button="left", click_count=2
            )
