...
from test_ai.self_improve.pr_manager import (
    ConflictResult,
    PRManager,
    PRStatus,
    PullRequest,
)
from test_ai.self_improve.rollback import RollbackManager, Snapshot
from test_ai.self_improve.orchestrator import (
    ImprovementPlan,
    ImprovementResult,
...
class TestCodebaseAnalyzer:
    ...
    def test_analyze_with_focus_paths(self, tmp_codebase: Path):
        ...
        analyzer = CodebaseAnalyzer(codebase_path=tmp_codebase)
        result = analyzer.analyze(focus_paths=["src/test_ai/sample.py"])
    ...
    def test_should_skip_file(self, tmp_codebase: Path):
        ...
        assert analyzer._should_skip_file(Path("src/test_ai/normal.py")) is False

    def test_check_code_quality_long_function(self, tmp_codebase: Path):
        """Detect functions longer than 50 lines."""
        analyzer = CodebaseAnalyzer(codebase_path=tmp_codebase)
        content = (tmp_codebase / "src" / "test_ai" / "bigfile.py").read_text()
    ...
    def test_check_documentation_missing_module_docstring(self, tmp_codebase: Path):
        ...
        analyzer = CodebaseAnalyzer(codebase_path=tmp_codebase)
        content = (tmp_codebase / "src" / "test_ai" / "no_doc.py").read_text()
    ...
    def test_check_documentation_missing_func_docstring(self, tmp_codebase: Path):
        ...
        suggestions = analyzer._check_documentation("no_doc.py", content)
        missing_func = [
            s for s in suggestions if "Missing docstring: undocumented" in s.title
        ]
    ...
    def test_check_documentation_skips_private_funcs(self, tmp_codebase: Path):
        ...
        analyzer = CodebaseAnalyzer(codebase_path=tmp_codebase)
        content = (tmp_codebase / "src" / "test_ai" / "sample.py").read_text()
    ...
    def test_analyze_file_read_error(self, tmp_codebase: Path):
        ...
        analyzer = CodebaseAnalyzer(codebase_path=tmp_codebase)
        fake_path = tmp_codebase / "src" / "test_ai" / "nonexistent.py"
    ...
    def test_analyze_with_ai_focus_file(self, tmp_codebase: Path):
        ...
        result = asyncio.run(
            analyzer.analyze_with_ai(focus_file="src/test_ai/sample.py")
        )
...
class TestRollbackManager:
    ...
    def test_create_snapshot_nonexistent_file(
        self, rollback_mgr: RollbackManager, tmp_path: Path
    ):
    ...
    def test_get_snapshot_loads_files_from_disk(
        self, snapshot_dir: Path, tmp_path: Path
    ):
    ...
    def test_rollback_creates_parent_dirs(
        self, rollback_mgr: RollbackManager, tmp_path: Path
    ):
    ...
    def test_save_and_load_index_roundtrip(self, snapshot_dir: Path, tmp_path: Path):
        ...
        mgr1.create_snapshot(
            files=["f.py"], description="persist test", codebase_path=code_dir
        )
...
class TestPRManager:
    ...
    def test_create_branch(self, pr_manager: PRManager):
        ...
        with patch.object(pr_manager, "_run_git") as mock_git:
            ...
            mock_git.assert_called_once_with(
                ["checkout", "-b", "test-improve/fix-docs"]
            )
    ...
    def test_commit_changes_success(self, pr_manager: PRManager):
        ...
        with patch.object(pr_manager, "_run_git", return_value=mock_result) as mock_git:
            commit_hash = pr_manager.commit_changes(
                files=["a.py", "b.py"], message="fix docs"
            )
    ...
    def test_push_branch_success(self, pr_manager: PRManager):
        ...
        with patch.object(pr_manager, "_run_git") as mock_git:
            ...
            mock_git.assert_called_once_with(
                ["push", "-u", "origin", "test-improve/fix"]
            )

    def test_push_branch_failure(self, pr_manager: PRManager):
        """Push failure returns False."""
        with patch.object(
            pr_manager, "_run_git", side_effect=RuntimeError("no remote")
        ):
    ...
    def test_create_pr_gh_success(self, pr_manager: PRManager):
        ...
        with patch(
            "test_ai.self_improve.pr_manager.subprocess.run", return_value=mock_result
        ):
    ...
    def test_create_pr_gh_not_found(self, pr_manager: PRManager):
        ...
        with patch(
            "test_ai.self_improve.pr_manager.subprocess.run",
    ...
    def test_create_pr_gh_exception(self, pr_manager: PRManager):
        ...
        with patch(
            ...
            pr = pr_manager.create_pr(
                branch="b", title="T", description="D", files_changed=[]
            )
            assert pr.url is None
    ...
    def test_mark_ready_for_review(self, pr_manager: PRManager):
        ...
        with patch("test_ai.self_improve.pr_manager.subprocess.run"):
    ...
    def test_delete_branch_failure(self, pr_manager: PRManager):
        ...
        with patch.object(
            pr_manager, "_run_git", side_effect=RuntimeError("not found")
        ):
...
class TestSelfImproveOrchestrator:
    ...
    def test_run_with_safety_violations(
        self, tmp_path: Path, safety_config: SafetyConfig
    ):
        ...
        config = SafetyConfig(
            critical_files=["src/test_ai/bad.py"],
            max_files_per_pr=50,
            max_lines_changed=5000,
            max_new_files=20,
            human_approval_plan=False,
    ...
    def test_run_full_workflow_no_approvals(
        self, tmp_path: Path, safety_config: SafetyConfig
    ):
    ...
    def test_create_plan_single_suggestion(
        self, tmp_path: Path, safety_config: SafetyConfig
    ):
    ...
    def test_create_plan_multiple_suggestions(
        self, tmp_path: Path, safety_config: SafetyConfig
    ):
