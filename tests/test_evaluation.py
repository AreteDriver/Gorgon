...
from test_ai.evaluation.metrics import (
    CodeExecutionMetric,
    CompositeMetric,
    ContainsMetric,
    ExactMatchMetric,
...
from test_ai.evaluation.reporters import (
    ConsoleReporter,
    HTMLReporter,
    JSONReporter,
    MarkdownReporter,
)
from test_ai.evaluation.runner import (
    EvalRunner,
    SuiteResult,
    create_quick_eval,
)
...
class TestProviderEvaluator:
    ...
    @patch("test_ai.evaluation.base.CompletionRequest", create=True)
    def test_evaluate_success(self, _mock_cr):
        ...
        with patch(
            "test_ai.providers.CompletionRequest",
    ...
    def test_evaluate_dict_input(self, _mock_cr):
        ...
        with patch("test_ai.providers.CompletionRequest", return_value=MagicMock()):
...
class TestEvalRunner:
    ...
    def test_run_async_progress_callback(self):
        callback_calls = []
        runner = self._make_runner(
            progress_callback=lambda c, t, r: callback_calls.append((c, t))
        )
        suite = self._make_suite_with_cases(2)
