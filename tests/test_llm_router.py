...
from test_ai.providers.router import (
    TierRouter,
    RoutingConfig,
    RoutingDecision,
    RoutingMode,
...
def manager():
    ...
    mock_openai.complete_async = AsyncMock(
        return_value=_mock_response("openai", "gpt-4o")
    )
    ...
    mock_ollama.complete_async = AsyncMock(
        return_value=_mock_response("ollama", "llama3.2")
    )
...
class TestBudgetAwareRouting:
    def test_budget_ok_routes_normally(self, manager):
        ...
        config = RoutingConfig(
            mode=RoutingMode.HYBRID, budget_force_local_threshold=5000
        )
...
class TestAsyncRouting:
    ...
    def test_async_fallback(self, manager):
        manager.get("openai").complete_async = AsyncMock(
            side_effect=ProviderError("openai down")
        )
        config = RoutingConfig(mode=RoutingMode.CLOUD)
        router = TierRouter(manager, config)
...
class TestEdgeCases:
    ...
    def test_routing_decision_defaults(self):
        d = RoutingDecision(
            provider_name="test", model=None, model_tier=None, reason="test"
        )
