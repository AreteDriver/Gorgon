...
class TestQuotaUsage:
    ...
    def test_reset_if_expired_minute(self):
        usage = QuotaUsage(
            ...
            limit=10,
            used=5,
            period_start=datetime.now(timezone.utc) - timedelta(seconds=120),
        )
    ...
    def test_reset_if_expired_hour(self):
        usage = QuotaUsage(
            ...
            limit=10,
            used=5,
            period_start=datetime.now(timezone.utc) - timedelta(hours=2),
        )
        assert usage.reset_if_expired() is True
        assert usage.used == 0
    ...
    def test_reset_if_expired_month(self):
        ...
        old_start = datetime.now(timezone.utc) - timedelta(days=60)
        usage = QuotaUsage(
...
class TestQuotaManager:
    ...
    def test_reset_all_periods(self):
        mgr = QuotaManager()
        mgr.configure(
            QuotaConfig(provider="test", requests_per_minute=10, requests_per_hour=100)
        )
        mgr.acquire("test", 5)
        mgr.reset("test")
        usage = mgr.get_usage("test")
