...
from test_ai.skills.consensus import (
    ConsensusLevel,
    ConsensusVerdict,
    ConsensusVoter,
    NUM_VOTERS,
...
from test_ai.skills.enforcer import (
    EnforcementAction,
    EnforcementResult,
    SkillEnforcer,
    Violation,
...
class TestConsensusVerdict:
    def test_approve_count_includes_abstains(self):
        ...
        verdict = ConsensusVerdict(
            level=ConsensusLevel.MAJORITY, approved=True, votes=votes
        )
...
class TestAggregate:
    ...
    def test_majority_two_reject(self):
        votes = self._make_votes(
            [VoteDecision.REJECT, VoteDecision.REJECT, VoteDecision.APPROVE]
        )
        verdict = ConsensusVoter._aggregate(ConsensusLevel.MAJORITY, votes)
        assert verdict.approved is False
    ...
    def test_majority_abstain_counts_as_approve(self):
        votes = self._make_votes(
            [VoteDecision.ABSTAIN, VoteDecision.APPROVE, VoteDecision.REJECT]
        )
        verdict = ConsensusVoter._aggregate(ConsensusLevel.MAJORITY, votes)
        assert verdict.approved is True
    ...
    def test_majority_one_approve_one_abstain_one_reject(self):
        votes = self._make_votes(
            [VoteDecision.APPROVE, VoteDecision.ABSTAIN, VoteDecision.REJECT]
        )
        verdict = ConsensusVoter._aggregate(ConsensusLevel.MAJORITY, votes)
    ...
    def test_majority_all_abstain(self):
        votes = self._make_votes(
            [VoteDecision.ABSTAIN, VoteDecision.ABSTAIN, VoteDecision.ABSTAIN]
        )
        verdict = ConsensusVoter._aggregate(ConsensusLevel.MAJORITY, votes)
    ...
    def test_unanimous_abstain_counts_as_approve(self):
        votes = self._make_votes(
            [VoteDecision.APPROVE, VoteDecision.APPROVE, VoteDecision.ABSTAIN]
        )
        verdict = ConsensusVoter._aggregate(ConsensusLevel.UNANIMOUS, votes)
        assert verdict.approved is True
...
class TestConsensusVoterCostTracking:
    def test_cost_tracking_called_on_success(self):
        with patch(
            "test_ai.skills.consensus.ConsensusVoter._track_voter_cost"
        ) as mock_track:
            client = MagicMock()
            client.generate_completion.return_value = {
                "success": True,
...
class TestSkillEnforcerCheckOutput:
    ...
    def test_protected_path_fnmatch_wildcard(self, enforcer_with_skills):
        ...
        result = enforcer.check_output(
            "builder", "Checking /etc/shadow for permissions"
        )
        assert result.passed is False
    ...
    def test_blocked_pattern_blocks_even_with_path_violation(
        self, enforcer_with_skills
    ):
