...
Targets:
- src/test_ai/monitoring/watchers.py (252 missed lines)
- src/test_ai/metrics/audit_checks.py (95 missed lines)
- src/test_ai/agents/supervisor.py (92 missed lines)
"""
...
# watchers.py imports
# ---------------------------------------------------------------------------
from test_ai.monitoring.watchers import (
    FileWatcher,
    LogWatcher,
    ResourceWatcher,
    WatchEvent,
...
# ---------------------------------------------------------------------------
from test_ai.agents.supervisor import (
    DelegationPlan,
    SupervisorAgent,
)
...
class TestCheckSkillIntegrity:
    def test_no_skills_path(self, audit_check: AuditCheck, baseline: SystemBaseline):
        result = asyncio.run(
            check_skill_integrity(
                audit_check, baseline=baseline, skills_path=Path("/nonexistent")
            )
        )
        assert result.status == AuditStatus.OK
        assert result.value == 0.0
    ...
    def test_oversized_file_skipped(
        ...
        with patch.object(Path, "stat", return_value=mock_stat):
            result = asyncio.run(
                check_skill_integrity(
                    audit_check, baseline=baseline, skills_path=tmp_path
                )
            )
...
class TestCheckConfigDrift:
    def test_no_config_path(self, audit_check: AuditCheck, baseline: SystemBaseline):
        result = asyncio.run(
            check_config_drift(
                audit_check, baseline=baseline, config_path=Path("/nonexistent")
            )
        )
        assert result.status == AuditStatus.OK
        assert result.value == 0.0
...
class TestCheckResourceUsage:
    def test_psutil_not_available(
        self, audit_check: AuditCheck, baseline: SystemBaseline
    ):
...
class TestRegisterDefaultChecks:
    def test_registers_all_checks(self, baseline: SystemBaseline):
        ...
        registered_names = [
            call.args[0] for call in auditor.register_check.call_args_list
        ]
...
class TestSupervisorAgentInit:
    def test_minimal_init(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        assert sup.provider is mock_provider
        assert sup.mode is None
        assert sup.session is None
        assert sup.backend is None
        assert sup._convergence_checker is None
        assert sup._skill_library is None
        assert sup._bridge is None
        assert sup._budget_manager is None
        assert sup._filesystem_tools is None
        assert sup._proposal_manager is None

    def test_filesystem_tools_init_with_session(self, mock_provider: MagicMock):
        session = MagicMock()
        session.has_project_access = True
        session.project_path = "/tmp/project"
        session.allowed_paths = []
        backend = MagicMock()

        with patch(
            "test_ai.agents.supervisor.SupervisorAgent._init_filesystem_tools"
        ) as mock_init:
            SupervisorAgent(provider=mock_provider, session=session, backend=backend)
            mock_init.assert_called_once()

    def test_no_filesystem_without_project_access(self, mock_provider: MagicMock):
        session = MagicMock()
        session.has_project_access = False
        backend = MagicMock()

        sup = SupervisorAgent(provider=mock_provider, session=session, backend=backend)
        assert sup._filesystem_tools is None


class TestBuildSystemPrompt:
    def test_base_prompt(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        prompt = sup._build_system_prompt()
        assert "Supervisor agent" in prompt
        assert "Self-Improvement Mode" not in prompt
        assert "Filesystem Tools" not in prompt

    def test_self_improve_mode(self, mock_provider: MagicMock):
        mode = MagicMock()
        mode.value = "self_improve"
        sup = SupervisorAgent(provider=mock_provider, mode=mode)
        prompt = sup._build_system_prompt()
        assert "Self-Improvement Mode" in prompt

    def test_non_self_improve_mode(self, mock_provider: MagicMock):
        mode = MagicMock()
    ...
    def test_filesystem_tools_in_prompt(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        sup._filesystem_tools = MagicMock()
        prompt = sup._build_system_prompt()
        assert "Filesystem Tools" in prompt


class TestBuildContext:
    def test_builds_context_list(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        msg = MagicMock()
        msg.role.value = "user"
        msg.content = "Hello"

        context = sup._build_context([msg])
        assert context[0]["role"] == "system"
        assert context[1]["role"] == "user"
        assert context[1]["content"] == "Hello"

    def test_includes_project_path(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        context = sup._build_context([], project_path="/my/project")
        assert any("Project context" in c["content"] for c in context)


# ===================================================================
# PART 17 — SupervisorAgent: _parse_delegation
# ===================================================================
...
class TestParseToolCalls:
    def test_valid_tool_call(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        response = '<tool_call>\n{"tool": "read_file", "path": "main.py"}\n</tool_call>'
        calls = sup._parse_tool_calls(response)
        assert len(calls) == 1
        assert calls[0]["tool"] == "read_file"

    def test_multiple_tool_calls(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        response = (
    ...
    def test_no_tool_calls(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        assert sup._parse_tool_calls("No tools here") == []

    def test_invalid_tool_call_json(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        response = "<tool_call>\nnot json\n</tool_call>"
        assert sup._parse_tool_calls(response) == []

    def test_tool_call_missing_tool_key(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        response = '<tool_call>\n{"path": "main.py"}\n</tool_call>'
        assert sup._parse_tool_calls(response) == []
...
# ===================================================================
# PART 19 — SupervisorAgent: _execute_tool_call
# ===================================================================


class TestExecuteToolCall:
    def test_no_filesystem_tools(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        result = asyncio.run(
            sup._execute_tool_call({"tool": "read_file", "path": "x.py"})
        )
        assert result["success"] is False
        assert "not available" in result["error"]

    def test_unknown_tool(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        sup._filesystem_tools = MagicMock()
        result = asyncio.run(sup._execute_tool_call({"tool": "unknown_tool"}))
        assert result["success"] is False
        assert "Unknown tool" in result["error"]

    def test_read_file_tool(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        mock_fs = MagicMock()
        mock_content = MagicMock()
        mock_content.model_dump.return_value = {"path": "main.py", "content": "x"}
        mock_fs.read_file.return_value = mock_content
        sup._filesystem_tools = mock_fs

        result = asyncio.run(
            sup._execute_tool_call(
                {
                    "tool": "read_file",
                    "path": "main.py",
                    "start_line": 1,
        ...
        mock_fs.read_file.assert_called_once_with("main.py", 1, 10)

    def test_list_files_tool(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        mock_fs = MagicMock()
        mock_listing = MagicMock()
        mock_listing.model_dump.return_value = {"files": []}
        mock_fs.list_files.return_value = mock_listing
        sup._filesystem_tools = mock_fs
    ...
    def test_search_code_tool(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        mock_fs = MagicMock()
        mock_search = MagicMock()
        mock_search.model_dump.return_value = {"matches": []}
        mock_fs.search_code.return_value = mock_search
        sup._filesystem_tools = mock_fs
    ...
    def test_get_structure_tool(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        mock_fs = MagicMock()
        mock_struct = MagicMock()
        mock_struct.model_dump.return_value = {"tree": {}}
        mock_fs.get_structure.return_value = mock_struct
        sup._filesystem_tools = mock_fs

        result = asyncio.run(
            sup._execute_tool_call({"tool": "get_structure", "max_depth": 3})
        )
        assert result["success"] is True

    def test_propose_edit_no_proposal_manager(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        sup._filesystem_tools = MagicMock()
        sup._proposal_manager = None
    ...
    def test_propose_edit_success(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        sup._filesystem_tools = MagicMock()
        sup._proposal_manager = MagicMock()
        sup.session = MagicMock()
        sup.session.id = "session-1"

        mock_proposal = MagicMock()
        mock_proposal.id = "prop-1"
        mock_proposal.file_path = "x.py"
        mock_proposal.status.value = "pending"
        sup._proposal_manager.create_proposal.return_value = mock_proposal
        ...
        assert result["success"] is True
        assert result["data"]["proposal_id"] == "prop-1"

    def test_tool_exception(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        mock_fs = MagicMock()
        mock_fs.read_file.side_effect = RuntimeError("file read failed")
        sup._filesystem_tools = mock_fs
    ...
    def test_audit_logging(self, mock_provider: MagicMock):
        sup = SupervisorAgent(provider=mock_provider)
        ...
        with patch("test_ai.tools.proposals.log_file_access"):
            result = asyncio.run(
                sup._execute_tool_call({"tool": "read_file", "path": "a.py"})
            )
...
# ===================================================================
# PART 20 — SupervisorAgent: _run_agent
# ===================================================================
...
# ===================================================================
# PART 21 — SupervisorAgent: _stream_response
# ===================================================================


class TestStreamResponse:
    def test_stream_completion(self, mock_provider: MagicMock):
        async def fake_stream(messages):
            yield "Hello "
            yield "World"

        mock_provider.stream_completion = fake_stream

        sup = SupervisorAgent(provider=mock_provider)

        async def collect():
            chunks = []
            async for chunk in sup._stream_response(
    ...
    def test_fallback_to_complete(self):
        provider = MagicMock()
        provider.complete = AsyncMock(return_value="Complete response")
        del provider.stream_completion
    ...
    def test_error_handling(self, mock_provider: MagicMock):
        async def bad_stream(messages):
            raise RuntimeError("Connection lost")
            yield  # noqa: F821 - needed to make this an async generator

        mock_provider.stream_completion = bad_stream
...
# ===================================================================
# PART 22 — SupervisorAgent: _execute_delegations
# ===================================================================
...
class TestExecuteDelegations:
    ...
    def test_convergence_drops_redundant(self, mock_provider: MagicMock):
        ...
        with patch(
            "test_ai.agents.convergence.format_convergence_alert", return_value="Alert!"
        ):
    ...
    def test_convergence_no_alert(self, mock_provider: MagicMock):
        ...
        with patch(
            "test_ai.agents.convergence.format_convergence_alert", return_value=None
        ):
            sup = SupervisorAgent(provider=mock_provider, convergence_checker=checker)
            results = asyncio.run(
                sup._execute_delegations(
...
class TestProcessMessage:
    def test_simple_response_no_delegation(self, mock_provider: MagicMock):
        async def fake_stream(messages):
            yield "Simple answer"
        ...
        async def collect():
            chunks = []
            async for chunk in sup.process_message("Hello", []):
                chunks.append(chunk)
            return chunks
    ...
    def test_delegation_response(self, mock_provider: MagicMock):
        delegation_json = json.dumps(
            {
                "analysis": "Need auth",
                "delegations": [{"agent": "builder", "task": "Build auth"}],
                "synthesis_approach": "Combine",
            }
        )
        response_with_delegation = f"```json\n{delegation_json}\n```"

        async def fake_stream(messages):
            yield response_with_delegation

        mock_provider.stream_completion = fake_stream
        mock_provider.complete = AsyncMock(return_value="Synthesized result")

        sup = SupervisorAgent(provider=mock_provider)

        # Patch _run_agent and _synthesize_results to avoid mock ordering issues
        sup._run_agent = AsyncMock(return_value="Built auth module")
        sup._synthesize_results = AsyncMock(return_value="Final synthesis")

        async def collect():
            chunks = []
            async for chunk in sup.process_message("Build auth", []):
                chunks.append(chunk)
            return chunks

        chunks = asyncio.run(collect())
        types = [c["type"] for c in chunks]
        assert "agent" in types  # Delegation notification
        assert "done" in types

    def test_tool_call_loop(self, mock_provider: MagicMock):
        call_count = 0
        ...
        async def fake_stream(messages):
            ...
            if call_count == 1:
                ...
                yield "Here is the file content analysis."
        ...
        mock_fs = MagicMock()
        mock_content = MagicMock()
        mock_content.model_dump.return_value = {"content": "file data"}
        mock_fs.read_file.return_value = mock_content
        sup._filesystem_tools = mock_fs

        async def collect():
            chunks = []
            async for chunk in sup.process_message("Read main.py", []):
                chunks.append(chunk)
            return chunks
...
# ===================================================================
# PART 26 — SupervisorAgent: skill library in delegations
# ===================================================================
