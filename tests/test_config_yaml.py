...
from test_ai.config.settings import (
    Settings,
...
def _make_settings(tmp_path, yaml_content=None, keep_env=(), **overrides):
    ...
    try:
        if yaml_path is not None:
            with patch(
                "test_ai.config.settings._find_yaml_config",
...
class TestFindYamlConfig:
    ...
    def test_returns_none_when_no_yaml_exists(self, tmp_path):
        ...
        with patch("test_ai.config.settings._YAML_SEARCH_PATHS", fake_paths):
    ...
    def test_returns_first_matching_path(self, tmp_path):
        ...
        with patch("test_ai.config.settings._YAML_SEARCH_PATHS", [first, second]):
    ...
    def test_ignores_directories(self, tmp_path):
        ...
        dir_path.mkdir()
        with patch("test_ai.config.settings._YAML_SEARCH_PATHS", [dir_path]):
            assert _find_yaml_config() is None
...
class TestYamlSettingsSource:
    ...
    def test_env_overrides_yaml(self, tmp_path, monkeypatch):
        ...
        s = _make_settings(
            tmp_path, yaml_content="log_level: DEBUG\n", keep_env=("LOG_LEVEL",)
        )
