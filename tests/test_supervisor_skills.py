...
class TestBuildSystemPromptWithSkills:
    ...
    def test_empty_routing_summary_not_appended(
        self, mock_provider, mock_skill_library
    ):
...
class TestExecuteDelegationsConsensus:
    ...
    def test_consensus_annotation_added(self, supervisor_with_skills, mock_provider):
        ...
        delegations = [
            {"agent": "system", "task": "Perform secure operations on files"}
        ]
        context = [{"role": "system", "content": "test"}]

        asyncio.run(
            supervisor_with_skills._execute_delegations(
                delegations, context, lambda x: None
            )
        )
