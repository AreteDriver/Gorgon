...
from test_ai.workflow.parallel import ParallelTask, ParallelStrategy
from test_ai.workflow.rate_limited_executor import (
    ProviderRateLimits,
...
class TestAdaptiveRateLimitState:
    ...
    def test_record_rate_limit_error_at_min(self):
        config = AdaptiveRateLimitConfig(
            backoff_factor=0.5, min_concurrent=5, cooldown_seconds=0
        )
        state = AdaptiveRateLimitState(
            base_limit=10,
            current_limit=5,
...
class TestRateLimitedExecutorExecution:
    def test_execute_parallel_asyncio(self):
        executor = RateLimitedParallelExecutor(
            strategy=ParallelStrategy.ASYNCIO, max_workers=2
        )
        tasks = [
    ...
    def test_execute_parallel_threading_fallback(self):
        executor = RateLimitedParallelExecutor(
            strategy=ParallelStrategy.THREADING, max_workers=2
        )
        tasks = [
            ParallelTask(id="t1", step_id="s1", handler=lambda: "r1"),
        ]
    ...
    def test_execute_fail_fast(self):
        ...
        tasks = [
            ...
            ParallelTask(
                id="t2", step_id="s2", handler=lambda: "ok", dependencies=["t1"]
            ),
        ]
