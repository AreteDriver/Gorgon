...
from test_ai.tui.session import (
    HISTORY_DIR,
    TUISession,
    _auto_title,
    _slugify,
...
class TestTUISession:
    ...
    def test_save_and_load(self, tmp_path, monkeypatch):
        monkeypatch.setattr("test_ai.tui.session.HISTORY_DIR", tmp_path)
        session = TUISession(
    ...
    def test_save_creates_directory(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("test_ai.tui.session.HISTORY_DIR", subdir)
    ...
    def test_list_sessions_nonexistent_dir(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "test_ai.tui.session.HISTORY_DIR", tmp_path / "does_not_exist"
        )
        assert TUISession.list_sessions() == []
