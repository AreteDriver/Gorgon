...
class TestInitProviders:
    ...
    @patch("test_ai.tui.app.create_provider_manager", create=True)
    def test_successful_init_with_provider(self, mock_create):
        ...
        with patch("test_ai.tui.providers.create_provider_manager", return_value=mgr):
    ...
    def test_no_default_provider_shows_error(self):
        ...
        assert (
            ...
            in cs.chat_display.add_error_message.call_args[0][0]
        )
    ...
    def test_provider_init_exception(self):
        ...
        with patch(
            "test_ai.tui.providers.create_provider_manager",
        ...
        cs.chat_display.add_error_message.assert_called_once()
        assert (
            "Provider init failed" in cs.chat_display.add_error_message.call_args[0][0]
        )
...
class TestInitCommands:
    ...
    def test_creates_registry_on_first_call(self):
        ...
        registry = MagicMock()
        with patch(
            "test_ai.tui.commands.create_command_registry", return_value=registry
        ):
    ...
    def test_skips_if_already_initialized(self):
        ...
        with patch("test_ai.tui.commands.create_command_registry") as mock_create:
            GorgonApp._init_commands(app)
...
class TestHandleChatMessage:
    ...
    def test_no_provider_manager_shows_error(self):
        ...
        cs.chat_display.add_error_message.assert_called_once_with(
            "No providers configured."
        )
    ...
    def test_successful_stream(self):
        ...
        with patch("test_ai.tui.streaming.stream_completion", side_effect=fake_stream):
    ...
    def test_streaming_error_shows_sanitized_error(self):
        ...
        with patch("test_ai.tui.streaming.stream_completion", side_effect=fail_stream):
            asyncio.run(GorgonApp._handle_chat_message(app, "hi"))
    ...
    def test_agent_mode_delegates(self):
        cs = _make_chat_screen_mock()
        app = _make_app(chat_screen=cs)
        provider = MagicMock()
        app._provider_manager = MagicMock()
        app._provider_manager.get_default.return_value = provider
        app._agent_mode = "auto"
        app._handle_agent_message = AsyncMock()

        asyncio.run(GorgonApp._handle_chat_message(app, "plan this"))

        cs.chat_display.add_user_message.assert_called_once_with("plan this")
        app._handle_agent_message.assert_awaited_once()
    ...
    def test_system_prompt_included(self):
        ...
        with patch(
            "test_ai.tui.streaming.stream_completion", side_effect=capture_stream
        ):
    ...
    def test_empty_response_not_appended(self):
        ...
        with patch("test_ai.tui.streaming.stream_completion", side_effect=empty_stream):
            asyncio.run(GorgonApp._handle_chat_message(app, "hello"))
...
# ------------------------------------------------------------------
# TestHandleAgentMessage
# ------------------------------------------------------------------


class TestHandleAgentMessage:
    """Test _handle_agent_message supervisor/agent streaming."""

    def _make_cs_app(self):
        cs = _make_chat_screen_mock()
        app = _make_app(chat_screen=cs)
        provider = MagicMock()
        app._provider_manager = MagicMock()
    ...
    def test_creates_supervisor_on_first_call(self):
        cs, app = self._make_cs_app()
        app._supervisor = None
        ...
        mock_sup.process_message = MagicMock(side_effect=fake_process)

        with (
            patch("test_ai.agents.provider_wrapper.AgentProvider"),
            patch(
                "test_ai.agents.supervisor.SupervisorAgent",
                return_value=mock_sup,
            ),
            patch(
                "test_ai.agents.convergence.create_checker",
                return_value=None,
            ),
            patch(
                "test_ai.agents.convergence.create_bridge",
                return_value=None,
            ),
            patch(
                "test_ai.budget.BudgetManager",
                return_value=MagicMock(),
            ),
        ):
            asyncio.run(GorgonApp._handle_agent_message(app, "plan it", cs))

        assert app._supervisor is mock_sup
        cs.chat_display.begin_assistant_stream.assert_called_once()
        cs.chat_display.append_stream_chunk.assert_called_with("hi")
        cs.chat_display.end_assistant_stream.assert_called_once_with("hi")

    def test_supervisor_init_failure(self):
        cs, app = self._make_cs_app()
        app._supervisor = None

        with patch(
            "test_ai.agents.provider_wrapper.AgentProvider",
            side_effect=ImportError("no module"),
        ):
            asyncio.run(GorgonApp._handle_agent_message(app, "go", cs))

        cs.chat_display.add_error_message.assert_called_once()
        assert "Agent init failed" in cs.chat_display.add_error_message.call_args[0][0]

    def test_text_chunk_handling(self):
        cs, app = self._make_cs_app()
        mock_sup = MagicMock()
        ...
        mock_sup.process_message = MagicMock(side_effect=fake_process)
        app._supervisor = mock_sup

        asyncio.run(GorgonApp._handle_agent_message(app, "test", cs))

        cs.chat_display.begin_assistant_stream.assert_called_once()
        assert cs.chat_display.append_stream_chunk.call_count == 2
        cs.chat_display.end_assistant_stream.assert_called_once_with("chunk1chunk2")
        assert app._messages[-1]["content"] == "chunk1chunk2"

    def test_agent_chunk_handling(self):
        cs, app = self._make_cs_app()
        mock_sup = MagicMock()

        async def fake_process(*args, **kwargs):
            yield {"type": "agent", "agent": "builder", "content": "building code"}
            yield {"type": "done", "content": ""}

        mock_sup.process_message = MagicMock(side_effect=fake_process)
        app._supervisor = mock_sup

        asyncio.run(GorgonApp._handle_agent_message(app, "build", cs))

        cs.chat_display.add_agent_message.assert_called_once_with(
            "builder", "building code"
        )
    ...
    def test_tool_result_handling(self):
        ...
        async def fake_process(*args, **kwargs):
            yield {"type": "tool_result", "content": "result data here"}
            yield {"type": "done", "content": ""}

        mock_sup.process_message = MagicMock(side_effect=fake_process)
        app._supervisor = mock_sup

        asyncio.run(GorgonApp._handle_agent_message(app, "tools", cs))

        cs.chat_display.add_system_message.assert_called_once()
        msg = cs.chat_display.add_system_message.call_args[0][0]
        assert msg.startswith("[tool] ")

    def test_done_chunk_breaks(self):
        cs, app = self._make_cs_app()
        mock_sup = MagicMock()
        ...
        cs.chat_display.append_stream_chunk.assert_not_called()

    def test_cancel_during_agent_stream(self):
        cs, app = self._make_cs_app()
        mock_sup = MagicMock()
        ...
        # Only "partial" should appear, not "more"
        chunk_calls = cs.chat_display.append_stream_chunk.call_args_list
        texts = [c[0][0] for c in chunk_calls]
        assert "partial" in texts
        assert "more" not in texts

    def test_agent_error_shows_sanitized(self):
        cs, app = self._make_cs_app()
        mock_sup = MagicMock()

        async def fail_process(*args, **kwargs):
            raise RuntimeError("API key sk-aaaabbbbccccddddeeeefffff leaked")
            yield  # makes this an async generator

        mock_sup.process_message = MagicMock(side_effect=fail_process)
        app._supervisor = mock_sup

        asyncio.run(GorgonApp._handle_agent_message(app, "fail", cs))

        cs.chat_display.add_error_message.assert_called_once()
        err = cs.chat_display.add_error_message.call_args[0][0]
        assert "sk-" not in err
        assert "***" in err
        assert app._is_streaming is False

    def test_reuses_existing_supervisor(self):
        cs, app = self._make_cs_app()
        existing = MagicMock()
        ...
        existing.process_message = MagicMock(side_effect=fake_process)
        app._supervisor = existing

        asyncio.run(GorgonApp._handle_agent_message(app, "test", cs))

        assert app._supervisor is existing

    def test_convergence_checker_failure_graceful(self):
        """If create_checker fails, supervisor still created with checker=None."""
        cs, app = self._make_cs_app()
        app._supervisor = None
        ...
        with (
            ...
            patch(
                "test_ai.agents.convergence.create_checker",
                side_effect=ImportError("no convergent"),
            ),
            patch(
                "test_ai.agents.convergence.create_bridge",
                return_value=None,
        ...
        # Supervisor should have been created with checker=None
        call_kwargs = sup_cls.call_args[1]
        assert call_kwargs["convergence_checker"] is None

    def test_no_full_response_no_message_appended(self):
        """When no text chunks received, no assistant message appended."""
        cs, app = self._make_cs_app()
        mock_sup = MagicMock()

        async def fake_process(*args, **kwargs):
            yield {"type": "agent", "agent": "planner", "content": "thinking..."}
            yield {"type": "done", "content": ""}
