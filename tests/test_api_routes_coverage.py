...
- src/test_ai/api_routes/executions.py
- src/test_ai/api_routes/budgets.py
- src/test_ai/api_routes/settings.py
- src/test_ai/api_routes/health.py
"""
...
class TestExecutionEndpoints:
    ...
    def test_get_execution_logs_with_level_filter(self, client, auth_headers):
        ...
        response = client.get(
            "/v1/executions/exec-1/logs?level=error", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_get_execution_logs_invalid_level(self, client, auth_headers):
        ...
        response = client.get(
            "/v1/executions/exec-1/logs?level=bogus", headers=auth_headers
        )
        assert response.status_code == 400
    ...
    def test_cleanup_executions_custom_hours(self, client, auth_headers):
        ...
        response = client.post(
            "/v1/executions/cleanup?max_age_hours=24", headers=auth_headers
        )
        assert response.status_code == 200
...
class TestBudgetEndpoints:
    ...
    def test_add_budget_usage_success(self, client, auth_headers):
        ...
        response = client.post(
            "/v1/budgets/bud-1/add-usage?amount=10.5", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_add_budget_usage_not_found(self, client, auth_headers):
        ...
        response = client.post(
            "/v1/budgets/nope/add-usage?amount=5", headers=auth_headers
        )
        assert response.status_code == 404
...
class TestHealthEndpoints:
    ...
    def test_database_health_check_sqlite(self, client, backend):
        with patch("test_ai.api_routes.health.get_database", return_value=backend):
            with patch(
                "test_ai.api_routes.health.get_migration_status",
    ...
    def test_database_health_check_postgres(self, client):
        from test_ai.state.backends import PostgresBackend
        ...
        with patch("test_ai.api_routes.health.get_database", return_value=mock_backend):
            with patch(
                "test_ai.api_routes.health.get_migration_status",
    ...
    def test_database_health_check_unknown_backend(self, client):
        ...
        with patch("test_ai.api_routes.health.get_database", return_value=mock_backend):
            with patch(
                "test_ai.api_routes.health.PostgresBackend", new=type("PG", (), {})
            ):
                with patch(
                    "test_ai.api_routes.health.SQLiteBackend",
                    new=type("SL", (), {}),
                ):
                    with patch(
                        "test_ai.api_routes.health.get_migration_status",
    ...
    def test_database_health_check_failure(self, client):
        with patch(
            "test_ai.api_routes.health.get_database",
    ...
    def test_full_health_check(self, client, backend):
        ...
        with patch("test_ai.api_routes.health.get_database", return_value=backend):
            with patch(
                "test_ai.api_routes.health.get_all_circuit_stats",
                return_value={},
            ):
                with patch(
                    "test_ai.api_routes.health.get_all_provider_stats",
                    return_value={},
                ):
    ...
    def test_metrics_endpoint(self, client):
        ...
        with patch(
            ...
            with patch("test_ai.metrics.get_collector", return_value=mock_collector):
                with patch(
                    "test_ai.metrics.PrometheusExporter",
