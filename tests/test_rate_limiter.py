...
class TestRateLimitResult:
    ...
    def test_remaining_under_limit(self):
        result = RateLimitResult(
            allowed=True, current_count=3, limit=10, reset_at=time.time() + 60
        )
    ...
    def test_retry_after_none_when_allowed(self):
        result = RateLimitResult(
            allowed=True, current_count=1, limit=10, reset_at=time.time() + 60
        )
...
class TestRateLimitResultEdgeCases:
    ...
    def test_zero_limit(self):
        result = RateLimitResult(
            allowed=False, current_count=1, limit=0, reset_at=time.time() + 60
        )
        assert result.remaining == 0
...
class TestMemoryRateLimiterConcurrency:
    ...
    def test_concurrent_acquires(self):
        ...
        async def acquire_many():
            tasks = [
                limiter.acquire("key", limit=100, window_seconds=3600)
                for _ in range(50)
            ]
    ...
    def test_rate_limiting_works_under_concurrency(self):
        ...
        async def acquire_many():
            tasks = [
                limiter.acquire("key", limit=5, window_seconds=3600) for _ in range(10)
            ]
            return await asyncio.gather(*tasks)
