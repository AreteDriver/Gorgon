...
from test_ai.executions.manager import ExecutionManager
from test_ai.jobs.job_manager import JobManager, JobStatus
from test_ai.orchestrator.workflow_engine import WorkflowResult
...
def mock_task_store():
    ...
    with patch("test_ai.db.get_task_store", return_value=mock_store):
        yield mock_store
...
def _make_job_manager(pipeline_backend, execution_manager, workflow_result=None):
    ...
    with patch("test_ai.jobs.job_manager.get_settings"):
        with patch(
            "test_ai.jobs.job_manager.get_database", return_value=pipeline_backend
        ):
            jm = JobManager(
...
class TestJobFailureRecordsError:
    ...
    def test_workflow_exception_marks_job_failed(
        self, pipeline_backend, execution_manager
    ):
        jm = _make_job_manager(pipeline_backend, execution_manager)
...
class TestMultipleConcurrentJobs:
    ...
    def test_three_concurrent_jobs_all_complete(
        ...
        try:
            jobs = [
                jm.submit(f"concurrent-wf-{i}", variables={"index": i})
                for i in range(3)
            ]
...
class TestCallbackExceptionDoesntBreakPipeline:
    ...
    def test_broken_callback_still_completes_job(
        self, pipeline_backend, execution_manager
    ):
