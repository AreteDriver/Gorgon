...
from test_ai.db import TaskStore, get_task_store, reset_task_store
...
class TestGetTaskStore:
    ...
    def test_singleton_behavior(self, backend):
        ...
        reset_task_store()
        with patch("test_ai.state.database.get_database", return_value=backend):
            s1 = get_task_store()
            s2 = get_task_store()
            assert s1 is s2
        reset_task_store()
    ...
    def test_uses_get_database(self, backend):
        ...
        reset_task_store()
        with patch(
            "test_ai.state.database.get_database", return_value=backend
        ) as mock_db:
            get_task_store()
