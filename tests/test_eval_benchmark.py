...
from test_ai.evaluation.base import (
    AgentEvaluator,
    EvalCase,
    EvalResult,
    EvalStatus,
...
from test_ai.evaluation.loader import METRIC_MAP, SuiteLoader, _build_metric
from test_ai.evaluation.metrics import ContainsMetric, LengthMetric
from test_ai.evaluation.runner import EvalRunner, SuiteResult
from test_ai.evaluation.store import EvalStore, get_eval_store, reset_eval_store
from test_ai.providers.mock_provider import MockProvider
...
def memory_backend():
    ...
    migration_path = (
        Path(__file__).parent.parent / "migrations" / "012_eval_results.sql"
    )
...
class TestEvalStore:
    ...
    def test_get_run(
        ...
        run_id = eval_store.record_run(
            "test_suite", sample_suite_result, agent_role="tester"
        )
    ...
    def test_feed_to_outcome_tracker(
        ...
        run_id = eval_store.record_run(
            "feed_suite", sample_suite_result, agent_role="tester"
        )

        mock_tracker = MagicMock()
        with (
            patch(
                "test_ai.state.database.get_database",
                return_value=eval_store.backend,
            ),
            patch(
                "test_ai.intelligence.outcome_tracker.OutcomeTracker",
    ...
    def test_feed_to_outcome_tracker_nonexistent_run(
...
class TestEvalCLI:
    ...
    @pytest.fixture()
    def cli_app(self):
        from test_ai.cli.commands.eval_cmd import eval_app
...
class TestEvalIntegration:
    ...
    def test_full_pipeline(self, tmp_suites_dir: Path, memory_backend) -> None:
        ...
        mock_provider = MockProvider(
            responses={"hello": "Hello, hi there! Nice to meet you."}
        )
    ...
    def test_reporter_output(self, tmp_suites_dir: Path) -> None:
        ...
        from test_ai.evaluation.reporters import ConsoleReporter, JSONReporter
...
class TestGlobalAccessors:
    ...
    def test_reset_eval_store(self) -> None:
        ...
        from test_ai.evaluation import store as store_mod
